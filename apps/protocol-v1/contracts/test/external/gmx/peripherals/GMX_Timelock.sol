...
import "./interfaces/GMX_ITimelockTarget.sol";
import "./interfaces/GMX_ITimelock.sol";
import "./interfaces/GMX_IHandlerTarget.sol";
import "../access/interfaces/GMX_IAdmin.sol";
import "../core/interfaces/GMX_IVault.sol";
import "../core/interfaces/GMX_IVaultUtils.sol";
import "../core/interfaces/GMX_IGlpManager.sol";
import "../referrals/interfaces/GMX_IReferralStorage.sol";
import "../tokens/interfaces/GMX_IYieldToken.sol";
import "../tokens/interfaces/GMX_IBaseToken.sol";
import "../tokens/interfaces/GMX_IMintable.sol";
import "../tokens/interfaces/GMX_IUSDG.sol";
import "../staking/interfaces/GMX_IVester.sol";
import "../staking/interfaces/GMX_IRewardRouterV2.sol";

import "../libraries/math/GMX_SafeMath.sol";
import "../libraries/token/GMX_IERC20.sol";

contract GMX_Timelock is GMX_ITimelock {
    ...
    function setExternalAdmin(address _target, address _admin) external onlyAdmin {
        ...
        GMX_IAdmin(_target).setAdmin(_admin);
    }
    ...
    function initGlpManager() external onlyAdmin {
        GMX_IGlpManager _glpManager = GMX_IGlpManager(glpManager);

        GMX_IMintable glp = GMX_IMintable(_glpManager.glp());
        glp.setMinter(glpManager, true);

        GMX_IUSDG usdg = GMX_IUSDG(_glpManager.usdg());
        usdg.addVault(glpManager);

        GMX_IVault vault = _glpManager.vault();
        vault.setManager(glpManager, true);
    }

    function initRewardRouter() external onlyAdmin {
        GMX_IRewardRouterV2 _rewardRouter = GMX_IRewardRouterV2(rewardRouter);

        GMX_IHandlerTarget(_rewardRouter.feeGlpTracker()).setHandler(rewardRouter, true);
        GMX_IHandlerTarget(_rewardRouter.stakedGlpTracker()).setHandler(rewardRouter, true);
        GMX_IHandlerTarget(glpManager).setHandler(rewardRouter, true);
    }
    ...
    function setMaxLeverage(address _vault, uint256 _maxLeverage) external onlyAdmin {
      ...
      GMX_IVault(_vault).setMaxLeverage(_maxLeverage);
    }
    ...
    function setFundingRate(address _vault, uint256 _fundingInterval, uint256 _fundingRateFactor, uint256 _stableFundingRateFactor) external onlyKeeperAndAbove {
        ...
        GMX_IVault(_vault).setFundingRate(_fundingInterval, _fundingRateFactor, _stableFundingRateFactor);
    }
    ...
    function setSwapFees(
        ...
        GMX_IVault vault = GMX_IVault(_vault);
    ...
    function setIsLeverageEnabled(address _vault, bool _isLeverageEnabled) external override onlyHandlerAndAbove {
        GMX_IVault(_vault).setIsLeverageEnabled(_isLeverageEnabled);
    }
    ...
    function setTokenConfig(
        ...
        GMX_IVault(_vault).setTokenConfig(
            _token,
            tokenDecimals,
            _tokenWeight,
            _minProfitBps,
        ...
        GMX_IVault(_vault).setBufferAmount(_token, _bufferAmount);

        GMX_IVault(_vault).setUsdgAmount(_token, _usdgAmount);
    }
    ...
    function setUsdgAmounts(address _vault, address[] memory _tokens, uint256[] memory _usdgAmounts) external onlyKeeperAndAbove {
        for (uint256 i = 0; i < _tokens.length; i++) {
            GMX_IVault(_vault).setUsdgAmount(_tokens[i], _usdgAmounts[i]);
        }
    }

    function updateUsdgSupply(uint256 usdgAmount) external onlyKeeperAndAbove {
        address usdg = GMX_IGlpManager(glpManager).usdg();
        uint256 balance = GMX_IERC20(usdg).balanceOf(glpManager);

        GMX_IUSDG(usdg).addVault(address(this));

        if (usdgAmount > balance) {
            uint256 mintAmount = usdgAmount.sub(balance);
            GMX_IUSDG(usdg).mint(glpManager, mintAmount);
        } else {
            uint256 burnAmount = balance.sub(usdgAmount);
            GMX_IUSDG(usdg).burn(glpManager, burnAmount);
        }

        GMX_IUSDG(usdg).removeVault(address(this));
    }

    function setShortsTrackerAveragePriceWeight(uint256 _shortsTrackerAveragePriceWeight) external onlyAdmin {
        GMX_IGlpManager(glpManager).setShortsTrackerAveragePriceWeight(_shortsTrackerAveragePriceWeight);
    }
    ...
    function setGlpCooldownDuration(uint256 _cooldownDuration) external onlyAdmin {
        ...
        GMX_IGlpManager(glpManager).setCooldownDuration(_cooldownDuration);
    }

    function setMaxGlobalShortSize(address _vault, address _token, uint256 _amount) external onlyAdmin {
        GMX_IVault(_vault).setMaxGlobalShortSize(_token, _amount);
    }

    function removeAdmin(address _token, address _account) external onlyAdmin {
        GMX_IYieldToken(_token).removeAdmin(_account);
    }

    function setIsSwapEnabled(address _vault, bool _isSwapEnabled) external onlyKeeperAndAbove {
        GMX_IVault(_vault).setIsSwapEnabled(_isSwapEnabled);
    }
    ...
    function batchSetBonusRewards(address _vester, address[] memory _accounts, uint256[] memory _amounts) external onlyKeeperAndAbove {
        ...
        GMX_IHandlerTarget(_vester).setHandler(address(this), true);

        for (uint256 i = 0; i < _accounts.length; i++) {
            address account = _accounts[i];
            uint256 amount = _amounts[i];
            GMX_IVester(_vester).setBonusRewards(account, amount);
        }

        GMX_IHandlerTarget(_vester).setHandler(address(this), false);
    }

    function transferIn(address _sender, address _token, uint256 _amount) external onlyAdmin {
        GMX_IERC20(_token).transferFrom(_sender, address(this), _amount);
    }
    ...
    function approve(address _token, address _spender, uint256 _amount) external onlyAdmin {
        ...
        _clearAction(action);
        GMX_IERC20(_token).approve(_spender, _amount);
    }
    ...
    function withdrawToken(address _target, address _token, address _receiver, uint256 _amount) external onlyAdmin {
        ...
        _validateAction(action);
        _clearAction(action);
        GMX_IBaseToken(_target).withdrawToken(_token, _receiver, _amount);
    }
    ...
    function setGov(address _target, address _gov) external onlyAdmin {
        ...
        _validateAction(action);
        _clearAction(action);
        GMX_ITimelockTarget(_target).setGov(_gov);
    }
    ...
    function setHandler(address _target, address _handler, bool _isActive) external onlyAdmin {
        ...
        _validateAction(action);
        _clearAction(action);
        GMX_IHandlerTarget(_target).setHandler(_handler, _isActive);
    }
    ...
    function setPriceFeed(address _vault, address _priceFeed) external onlyAdmin {
        ...
        _validateAction(action);
        _clearAction(action);
        GMX_IVault(_vault).setPriceFeed(_priceFeed);
    }
    ...
    function redeemUsdg(address _vault, address _token, uint256 _amount) external onlyAdmin {
        ...
        address usdg = GMX_IVault(_vault).usdg();
        GMX_IVault(_vault).setManager(address(this), true);
        GMX_IUSDG(usdg).addVault(address(this));

        GMX_IUSDG(usdg).mint(address(this), _amount);
        GMX_IERC20(usdg).transfer(address(_vault), _amount);

        GMX_IVault(_vault).sellUSDG(_token, mintReceiver);

        GMX_IVault(_vault).setManager(address(this), false);
        GMX_IUSDG(usdg).removeVault(address(this));
    }
    ...
    function _mint(address _token, address _receiver, uint256 _amount) private {
        GMX_IMintable mintable = GMX_IMintable(_token);

        mintable.setMinter(address(this), true);

        mintable.mint(_receiver, _amount);
        require(GMX_IERC20(_token).totalSupply() <= maxTokenSupply, "Timelock: maxTokenSupply exceeded");
