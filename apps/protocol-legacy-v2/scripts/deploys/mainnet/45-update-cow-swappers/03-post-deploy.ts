...
async function setupSusdsPlus() {
  ...
  {
    await mine(
      INSTANCES.VAULTS.SUSDSpS.COW_SWAPPER_3.setOrderConfig(
    ...
    await mine(
      INSTANCES.VAULTS.SUSDSpS.COW_SWAPPER_3.setCowApproval(
        ADDRS.EXTERNAL.SKY.SKY_TOKEN, 
    ...
    await mine(
      INSTANCES.VAULTS.SUSDSpS.COW_SWAPPER_3.createConditionalOrder(ADDRS.EXTERNAL.SKY.SKY_TOKEN)
    );
  }
  ...
  // SPK rewards
  {
    ...
    await mine(
      INSTANCES.VAULTS.SUSDSpS.COW_SWAPPER_3.createConditionalOrder(ADDRS.EXTERNAL.SPARK.SPK_TOKEN)
    );
  }
}
...
async function setupSkyPlus() {
  ...
  {
    await mine(
      INSTANCES.VAULTS.SKYp.COW_SWAPPER_2.setOrderConfig(
    ...
    await mine(
      INSTANCES.VAULTS.SKYp.COW_SWAPPER_2.setCowApproval(
        ADDRS.EXTERNAL.SKY.USDS_TOKEN, 
        ethers.constants.MaxUint256
      )
    );
    ...
    await mine(
      INSTANCES.VAULTS.SKYp.COW_SWAPPER_2.createConditionalOrder(ADDRS.EXTERNAL.SKY.USDS_TOKEN)
    );
  }
  ...
  // SPK rewards
  {
    ...
    await mine(
      INSTANCES.VAULTS.SKYp.COW_SWAPPER_2.createConditionalOrder(ADDRS.EXTERNAL.SPARK.SPK_TOKEN)
    );
  }
}
...
async function main() {
  ...
  await mine(INSTANCES.VAULTS.SUSDSpS.COW_SWAPPER_3.proposeNewOwner(ADDRS.CORE.MULTISIG));

  await setupSkyPlus();
  await mine(INSTANCES.VAULTS.SKYp.COW_SWAPPER_2.proposeNewOwner(ADDRS.CORE.MULTISIG));

  const batch = createSafeBatch(
    [
      setSwapper(INSTANCES.VAULTS.SUSDSpS.MANAGER, ADDRS.VAULTS.SUSDSpS.COW_SWAPPER_3),
      setSwapper(INSTANCES.VAULTS.SKYp.MANAGER, ADDRS.VAULTS.SKYp.COW_SWAPPER_2),

      acceptOwner(INSTANCES.VAULTS.SUSDSpS.COW_SWAPPER_3),
      acceptOwner(INSTANCES.VAULTS.SKYp.COW_SWAPPER_2),
    ],
  );
