...
const getEncodedPrices = (ADDRS: ContractAddresses) => (
  {
    ...
    oac_ibera_ibgt_toUsd: encodedErc4626TokenPrice(ADDRS.VAULTS.INFRARED_AUTO_COMPOUNDER_IBERA_IBGT_A.TOKEN.address),
  }
);
...
function updatePricesSafeBatch(contract: TokenPrices, ADDRS: ContractAddresses): SafeTransaction[] {
  ...
  return [
    ...
    setTokenPriceFunction(contract, ADDRS.VAULTS.INFRARED_AUTO_COMPOUNDER_IBERA_IBGT_A.TOKEN.address,
...
async function main() {
  ...
  writeSafeTransactionsBatch(
    createSafeBatch([
      acceptOwnerAddr(ADDRS.VAULTS.INFRARED_AUTO_COMPOUNDER_IBERA_IBGT_A.TOKEN.address),
      ...
      acceptOwnerAddr(ADDRS.VAULTS.INFRARED_AUTO_STAKING_IBERA_IBGT_A.VAULT.address),
