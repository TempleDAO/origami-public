...
async function main() {
    ...
    const autoCompoundingSeed = await seedAutoCompoundingVaultMsig(
      owner,
      ADDRS.VAULTS.INFRARED_AUTO_COMPOUNDER_SWBERA_OSBGT_A.TOKEN.address,
      ADDRS.EXTERNAL.INFRARED.REWARD_VAULTS.SWBERA_OSBGT,
      DEFAULT_SETTINGS.VAULTS.INFRARED_AUTO_COMPOUNDERS.SWBERA_OSBGT.SEED_DEPOSIT_SIZE,
      ADDRS.CORE.MULTISIG,
    );

    const autoStakingSeed = await seedAutoStakingVaultMsig(
      owner, 
      ADDRS.VAULTS.INFRARED_AUTO_STAKING_SWBERA_OSBGT_A.VAULT.address,
      ADDRS.EXTERNAL.INFRARED.REWARD_VAULTS.SWBERA_OSBGT,
      DEFAULT_SETTINGS.VAULTS.INFRARED_AUTO_STAKING.SWBERA_OSBGT.SEED_DEPOSIT_SIZE,
    )
