...
async function main() {
  ...
  const pendleMarketAddress = ADDRS.EXTERNAL.PENDLE.SUSDE_MAY_2025.MARKET;

  // Only use 15mins in localhost since we move forward in time and the Chainlink
  // oracles will become stale if too long
  const twapSecs = network.name === "localhost" ? 900 : DEFAULT_SETTINGS.ORACLES.PT_SUSDE_MAY_2025_SUSDE.TWAP_DURATION_SECS;
  ...
  await deployAndMine(
    'ORACLES.PT_SUSDE_MAY_2025_USDE',
    factory,
    factory.deploy,
    {
      description: "PT-sUSDe-May2025/USDe",
      baseAssetAddress: ADDRS.EXTERNAL.PENDLE.SUSDE_MAY_2025.PT_TOKEN,
      baseAssetDecimals: await INSTANCES.EXTERNAL.PENDLE.SUSDE_MAY_2025.PT_TOKEN.decimals(),
