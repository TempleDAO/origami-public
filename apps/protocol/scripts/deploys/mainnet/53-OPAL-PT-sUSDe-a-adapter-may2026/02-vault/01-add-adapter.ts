...
function getEncodedMappings() {
  const usde_per_ptSusde = encodedOrigamiOraclePrice(
    ADDRS.ORACLES.PT_SUSDE_7MAY2026_USDE,
  ...
  return [
    {
      token: ADDRS.EXTERNAL.PENDLE.SUSDE_7MAY2026.PT_TOKEN,
...
async function main() {
  ...
  const collateralToken = INSTANCES.EXTERNAL.PENDLE.SUSDE_7MAY2026.PT_TOKEN;
  const debtToken = INSTANCES.EXTERNAL.CIRCLE.USDC_TOKEN;
  const emode = DEFAULT_SETTINGS.EXTERNAL.AAVE.CORE.EMODES.PT_sUSDe_7MAY2026__Stablecoins;
  const maxUrOnJoin = vaultSettings.MAX_UR_ON_JOIN["AAVE: PT-sUSDe-7MAY2026 / USDC"];
  ...
  if (network.name == 'localhost') {
    const managerOwner = await impersonateAndFund2(await INSTANCES.VAULTS.OPAL_PT_SUSDE_A_DEPRECATED.MANAGER.owner());
