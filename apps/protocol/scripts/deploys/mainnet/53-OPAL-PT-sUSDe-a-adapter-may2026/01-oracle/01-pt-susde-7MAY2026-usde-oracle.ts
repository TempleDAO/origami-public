...
async function main() {
  ...
  const pendleMarketAddress = ADDRS.EXTERNAL.PENDLE.SUSDE_7MAY2026.MARKET;

  // Only use 15mins in localhost since we move forward in time and the Chainlink
  // oracles will become stale if too long
  const twapSecs = network.name === "localhost" ? 900 : DEFAULT_SETTINGS.ORACLES.PT_SUSDE_7MAY2026_SUSDE.TWAP_DURATION_SECS;
  ...
  await deployAndMine(
    'ORACLES.PT_SUSDE_7MAY2026_USDE',
    factory,
    factory.deploy,
    {
      description: "PT-sUSDe-7MAY2026/USDe",
      baseAssetAddress: ADDRS.EXTERNAL.PENDLE.SUSDE_7MAY2026.PT_TOKEN,
      baseAssetDecimals: await INSTANCES.EXTERNAL.PENDLE.SUSDE_7MAY2026.PT_TOKEN.decimals(),
