...
const SUSDE_WHALE = "0x31d3243cfb54b34fc9c73e1cb1137124bd6b13e1"; // sifuvision.eth :-)
...
async function investLov_sUSDe(
  ...
  const quoteData = await INSTANCES.LOV_SUSDE_B.TOKEN.investQuote(
    amountBN,
  ...
  await mine(
    INSTANCES.LOV_SUSDE_B.TOKEN.connect(account).investWithToken(
      quoteData.quoteData,
      {gasLimit:5000000}
    )
  );
  ...
  console.log("\tAccount balance of lov-sUSDe:", ethers.utils.formatEther(
    await INSTANCES.LOV_SUSDE_B.TOKEN.balanceOf(account.getAddress())
  ));
}
...
async function exitLov_sUSDe(
  ...
  // the share price
  const slippageBps = 1;
  const quoteData = await INSTANCES.LOV_SUSDE_B.TOKEN.exitQuote(
    amountBN,
    ADDRS.EXTERNAL.ETHENA.SUSDE_TOKEN,
    slippageBps, 
    0
  ...
  await mine(
    INSTANCES.LOV_SUSDE_B.TOKEN.connect(account).exitToToken(
      quoteData.quoteData,
      account.getAddress(),
      {gasLimit:5000000}
    )
  ...
  console.log("\t\tmaxExit afterwards:", ethers.utils.formatEther(
    await INSTANCES.LOV_SUSDE_B.TOKEN.maxExit(ADDRS.EXTERNAL.ETHENA.SUSDE_TOKEN)
  ));
}
...
async function solveRebalanceDownAmount(
  ...
  */
  const [assets, liabilities, ] = await INSTANCES.LOV_SUSDE_B.MANAGER.assetsAndLiabilities(PriceType.SPOT_PRICE);
...
function supplyTokenToDebtTokenQuote(fromAmount: BigNumber) {
  ...
  */
  ...
  if (fromAmount.eq(ethers.utils.parseEther("200000"))) {
    const toAmount = ethers.utils.parseEther("205289.935328386225739321");
    return {
      toAmount,
      price: toAmount.mul(ONE_ETHER).div(fromAmount),
      data: ""
    };
  } else {
...
function debtTokenToSupplyTokenQuote(fromAmount: BigNumber) {
  /*
    curl -X GET \
    "https://api.1inch.dev/swap/v6.0/1/swap?src=0x6B175474E89094C44Da98b954EedeAC495271d0F&dst=0x9D39A5DE30e57443BfF2A8307A4256c8797A3497&amount=218428798151854675786324&from=0x0000000000000000000000000000000000000000&slippage=50&disableEstimate=true&connectorTokens=0x83F20F44975D03b1b09e64809B757c47f942BEeA" \
    -H "Authorization: Bearer PinnqIP4n9rxYRndzIyWDVrMfmGKUbZG" \
    -H "accept: application/json" \
    -H "content-type: application/json"
  */

  if (fromAmount.eq(ethers.utils.parseEther("218428.798151854675786324"))) {
    const toAmount = ethers.utils.parseEther("205263.605817120075577056");
    return {
      toAmount,
      price: fromAmount.mul(ONE_ETHER).div(toAmount),
      data: ""
    };
  } else {
...
async function rebalanceDown(
  ...
  const alRatioBefore = await INSTANCES.LOV_SUSDE_B.MANAGER.assetToLiabilityRatio();
  ...
  await mine(
    INSTANCES.LOV_SUSDE_B.MANAGER.rebalanceDown(
      {
  ...
  const alRatioAfter = await INSTANCES.LOV_SUSDE_B.MANAGER.assetToLiabilityRatio();
...
async function increaseMaxSupply(investAmount: BigNumber) {
  await INSTANCES.LOV_SUSDE_B.TOKEN.setMaxTotalSupply(investAmount);
}

async function supplyDaiIntoMorpho(owner: SignerWithAddress) {
  const supplyAmount = ethers.utils.parseEther("9000000");
  ...
  await mine(
    INSTANCES.EXTERNAL.MORPHO.SINGLETON.connect(signer).supply(
      await INSTANCES.LOV_SUSDE_B.MORPHO_BORROW_LEND.getMarketParams(),
      supplyAmount,
      0,
...
async function main() {
  ...
  const maxExitAmount = await INSTANCES.LOV_SUSDE_B.TOKEN.maxExit(ADDRS.EXTERNAL.ETHENA.SUSDE_TOKEN);
