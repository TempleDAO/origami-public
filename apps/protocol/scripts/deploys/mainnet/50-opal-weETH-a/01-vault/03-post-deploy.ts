import { encodedMulPrice, encodedOraclePrice, encodedTokenizedBalanceSheetTokenPrice, impersonateAndFund2, mine, runAsyncMain } from "../../../helpers";
...
function getEncodedMappings(opalVault: IOpalVault) {
  // Use chainlink ETH/USD
  const usd_per_eth = encodedOraclePrice(
    ADDRS.EXTERNAL.CHAINLINK.ETH_USD_ORACLE,
    DEFAULT_SETTINGS.EXTERNAL.CHAINLINK.ETH_USD_ORACLE.STALENESS_THRESHOLD
  );

  // Chainlink weETH/ETH
  const weth_per_weeth = encodedOraclePrice(
    ADDRS.EXTERNAL.CHAINLINK.WEETH_ETH_ORACLE,
    DEFAULT_SETTINGS.EXTERNAL.CHAINLINK.WEETH_ETH_ORACLE.STALENESS_THRESHOLD
  );
  
  // Use the same rates so the ratio is consistent.
  const usd_per_weeth = encodedMulPrice(weth_per_weeth, usd_per_eth);
  
  return [
    {
      token: ADDRS.EXTERNAL.WETH_TOKEN,
      fnCalldata: usd_per_eth,
    },
    {
      token: ADDRS.EXTERNAL.ETHERFI.WEETH_TOKEN,
      fnCalldata: usd_per_weeth,
    },
    {
...
async function main() {
  ...
  const vaultSettings = DEFAULT_SETTINGS.VAULTS.OPAL_WEETH_A_DEPRECATED;
  const opalVault = INSTANCES.VAULTS.OPAL_WEETH_A_DEPRECATED;
