...
const COLLATERAL_WHALE = '0xdbf4FB6C310C1C85D0b41B5DbCA06096F2E7099F';
...
const getContracts = async (vault: OpalVault): Promise<OpalContracts> => {
  ...
  return {
    ...
    TOKEN_PRICES: INSTANCES.CORE.TOKEN_PRICES.V4,
  };
}
...
async function main() {
  ...
  OPAL_INSTANCES = await getContracts(INSTANCES.VAULTS.OPAL_PT_SUSDE_A_DEPRECATED.TOKEN);
  const vaultSettings = DEFAULT_SETTINGS.VAULTS.OPAL_PT_SUSDE_A_DEPRECATED;

  console.log("\nToken Prices Before Seed:");
  await dumpPrices(INSTANCES.CORE.TOKEN_PRICES.V4);
