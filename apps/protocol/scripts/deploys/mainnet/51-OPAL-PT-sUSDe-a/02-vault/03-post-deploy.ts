...
function getEncodedMappings(opalVault: IOpalVault) {
  const usde_per_ptSusde = encodedOrigamiOraclePrice(
    ADDRS.ORACLES.PT_SUSDE_FEB_2026_USDE,
  ...
  return [
    {
      token: ADDRS.EXTERNAL.CIRCLE.USDC_TOKEN,
    ...
    {
      token: ADDRS.EXTERNAL.PENDLE.SUSDE_FEB_2026.PT_TOKEN,
...
async function main() {
  ...
  const vaultSettings = DEFAULT_SETTINGS.VAULTS.OPAL_PT_SUSDE_A_DEPRECATED;
  const opalVault = INSTANCES.VAULTS.OPAL_PT_SUSDE_A_DEPRECATED;
  ...
  const debtToken0 = INSTANCES.EXTERNAL.CIRCLE.USDC_TOKEN;
  const emode0 = DEFAULT_SETTINGS.EXTERNAL.AAVE.CORE.EMODES["PT-sUSDe Stablecoins Nov 2025"];
  const maxUrOnJoin0 = vaultSettings.MAX_UR_ON_JOIN["AAVE: sUSDe / USDC"];

  const collateralToken1 = INSTANCES.EXTERNAL.PENDLE.SUSDE_FEB_2026.PT_TOKEN;
  const debtToken1 = INSTANCES.EXTERNAL.CIRCLE.USDC_TOKEN;
  const emode1 = DEFAULT_SETTINGS.EXTERNAL.AAVE.CORE.EMODES["PTsUSDe5FEB/Stablecoins"];
  const maxUrOnJoin1 = vaultSettings.MAX_UR_ON_JOIN["AAVE: PT-sUSDe-Feb-2026 / USDC"];
