...
async function updatePricesSafeBatch(contract: TokenPrices) {
  const batch = createSafeBatch(
    ...
    [
      ...
      setTokenPriceFunction(contract, ADDRS.EXTERNAL.MAKER_DAO.SDAI_TOKEN,
        encodedMulPrice(
          encodedErc4626TokenPrice(ADDRS.EXTERNAL.MAKER_DAO.SDAI_TOKEN),
          encodedOraclePrice(
            ADDRS.EXTERNAL.CHAINLINK.DAI_USD_ORACLE, 
...
async function main() {
  ...
  await mine(
    INSTANCES.LOV_WETH_SDAI_SHORT_A.SPARK_BORROW_LEND.setPositionOwner(ADDRS.LOV_WETH_SDAI_SHORT_A.MANAGER),
  );

  await mine(
    INSTANCES.LOV_WETH_SDAI_SHORT_A.MANAGER.setOracles(
  ...
  await mine(
    INSTANCES.LOV_WETH_SDAI_SHORT_A.MANAGER.setUserALRange(
      DEFAULT_SETTINGS.LOV_WETH_SDAI_SHORT_A.USER_AL_FLOOR,
      DEFAULT_SETTINGS.LOV_WETH_SDAI_SHORT_A.USER_AL_CEILING
    )
  );
  await mine(
    INSTANCES.LOV_WETH_SDAI_SHORT_A.MANAGER.setRebalanceALRange(
      DEFAULT_SETTINGS.LOV_WETH_SDAI_SHORT_A.REBALANCE_AL_FLOOR,
      DEFAULT_SETTINGS.LOV_WETH_SDAI_SHORT_A.REBALANCE_AL_CEILING
    )
  );
  await mine(
    INSTANCES.LOV_WETH_SDAI_SHORT_A.MANAGER.setSwapper(
      ADDRS.SWAPPERS.DIRECT_SWAPPER
    )
  );

  await mine(
    INSTANCES.LOV_WETH_SDAI_SHORT_A.MANAGER.setFeeConfig(
      DEFAULT_SETTINGS.LOV_WETH_SDAI_SHORT_A.MIN_DEPOSIT_FEE_BPS,
      DEFAULT_SETTINGS.LOV_WETH_SDAI_SHORT_A.MIN_EXIT_FEE_BPS,
      DEFAULT_SETTINGS.LOV_WETH_SDAI_SHORT_A.FEE_LEVERAGE_FACTOR
    )
  );

  await mine(
    INSTANCES.LOV_WETH_SDAI_SHORT_A.TOKEN.setManager(
      ADDRS.LOV_WETH_SDAI_SHORT_A.MANAGER
    )
  );
  
  await mine(
    INSTANCES.LOV_WETH_SDAI_SHORT_A.MANAGER.setAllowAll(
      true
    )
  );
