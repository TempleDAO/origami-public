...
async function seedDepositTestnet(
  ...
  const vault = INSTANCES.VAULTS.OAC_USDS_IMF_MOR.TOKEN;
  const vaultDecimals = await vault.decimals();
  const assetToken = INSTANCES.EXTERNAL.SKY.USDS_TOKEN;
...
async function main() {
  ...
  if (network.name === 'localhost') {
    ...
    await mine(INSTANCES.EXTERNAL.SKY.USDS_TOKEN.connect(signer).transfer(
      owner.getAddress(),
      DEFAULT_SETTINGS.VAULTS.OAC_USDS_IMF_MOR.SEED_DEPOSIT_SIZE
    ));

    await seedDepositTestnet(
      DEFAULT_SETTINGS.VAULTS.OAC_USDS_IMF_MOR.SEED_DEPOSIT_SIZE,
      await owner.getAddress(), 
      DEFAULT_SETTINGS.VAULTS.OAC_USDS_IMF_MOR.MAX_TOTAL_SUPPLY
    );
  } else {
