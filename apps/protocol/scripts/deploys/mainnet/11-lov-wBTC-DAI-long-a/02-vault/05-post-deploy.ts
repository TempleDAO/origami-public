...
async function updatePricesSafeBatch(contract: TokenPrices) {
  const batch = createSafeBatch(
    ...
    [
      ...
      setTokenPriceFunction(contract, ADDRS.EXTERNAL.WBTC_TOKEN,
        encodedOraclePrice(
          ADDRS.EXTERNAL.CHAINLINK.BTC_USD_ORACLE, 
          DEFAULT_SETTINGS.EXTERNAL.CHAINLINK.BTC_USD_ORACLE.STALENESS_THRESHOLD
        )
...
async function main() {
  ...
  await mine(
    INSTANCES.LOV_WBTC_DAI_LONG_A.SPARK_BORROW_LEND.setPositionOwner(ADDRS.LOV_WBTC_DAI_LONG_A.MANAGER),
  );

  await mine(
    INSTANCES.LOV_WBTC_DAI_LONG_A.MANAGER.setOracles(
  ...
  await mine(
    INSTANCES.LOV_WBTC_DAI_LONG_A.MANAGER.setUserALRange(
      DEFAULT_SETTINGS.LOV_WBTC_DAI_LONG_A.USER_AL_FLOOR,
      DEFAULT_SETTINGS.LOV_WBTC_DAI_LONG_A.USER_AL_CEILING
    )
  );
  await mine(
    INSTANCES.LOV_WBTC_DAI_LONG_A.MANAGER.setRebalanceALRange(
      DEFAULT_SETTINGS.LOV_WBTC_DAI_LONG_A.REBALANCE_AL_FLOOR,
      DEFAULT_SETTINGS.LOV_WBTC_DAI_LONG_A.REBALANCE_AL_CEILING
    )
  );
  await mine(
    INSTANCES.LOV_WBTC_DAI_LONG_A.MANAGER.setSwapper(
      ADDRS.SWAPPERS.DIRECT_SWAPPER
    )
  );

  await mine(
    INSTANCES.LOV_WBTC_DAI_LONG_A.MANAGER.setFeeConfig(
      DEFAULT_SETTINGS.LOV_WBTC_DAI_LONG_A.MIN_DEPOSIT_FEE_BPS,
      DEFAULT_SETTINGS.LOV_WBTC_DAI_LONG_A.MIN_EXIT_FEE_BPS,
      DEFAULT_SETTINGS.LOV_WBTC_DAI_LONG_A.FEE_LEVERAGE_FACTOR
    )
  );

  await mine(
    INSTANCES.LOV_WBTC_DAI_LONG_A.TOKEN.setManager(
      ADDRS.LOV_WBTC_DAI_LONG_A.MANAGER
    )
  );
  
  await mine(
    INSTANCES.LOV_WBTC_DAI_LONG_A.MANAGER.setAllowAll(
      true
    )
  );
