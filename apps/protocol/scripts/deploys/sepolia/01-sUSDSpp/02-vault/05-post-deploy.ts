...
async function updatePricesSafeBatch(contract: TokenPrices) {
  ...
  const batch = createSafeBatch(
    ...
    [
      ...
      setTokenPriceFunction(contract, ADDRS.VAULTS.SUSDSpp.TOKEN,
        encodedPrices.vaultTokenToUsd
      ),
    ],
  );
...
async function setupCowSwapper() {
  await mine(
    INSTANCES.VAULTS.SUSDSpp.COW_SWAPPER.setOrderConfig(
      ADDRS.EXTERNAL.SKY.SKY_TOKEN,
      {
        maxSellAmount: DEFAULT_SETTINGS.VAULTS.SUSDSpp.COW_SWAPPERS.EXACT_SKY_TO_USDS.MAX_SELL_AMOUNT,
        buyToken: ADDRS.EXTERNAL.SKY.USDS_TOKEN,
        minBuyAmount: DEFAULT_SETTINGS.VAULTS.SUSDSpp.COW_SWAPPERS.EXACT_SKY_TO_USDS.MIN_BUY_AMOUNT,
        limitPriceOracle: ZERO_ADDRESS,
        roundDownDivisor: DEFAULT_SETTINGS.VAULTS.SUSDSpp.COW_SWAPPERS.EXACT_SKY_TO_USDS.ROUND_DOWN_DIVISOR,
        partiallyFillable: DEFAULT_SETTINGS.VAULTS.SUSDSpp.COW_SWAPPERS.EXACT_SKY_TO_USDS.PARTIALLY_FILLABLE,
        useCurrentBalanceForSellAmount: DEFAULT_SETTINGS.VAULTS.SUSDSpp.COW_SWAPPERS.EXACT_SKY_TO_USDS.USE_CURRENT_BALANCE_FOR_SELL_AMOUNT,
        limitPricePremiumBps: DEFAULT_SETTINGS.VAULTS.SUSDSpp.COW_SWAPPERS.EXACT_SKY_TO_USDS.LIMIT_PRICE_PREMIUM_BPS,
        verifySlippageBps: DEFAULT_SETTINGS.VAULTS.SUSDSpp.COW_SWAPPERS.EXACT_SKY_TO_USDS.VERIFY_SLIPPAGE_BPS,
        expiryPeriodSecs: DEFAULT_SETTINGS.VAULTS.SUSDSpp.COW_SWAPPERS.EXACT_SKY_TO_USDS.EXPIRY_PERIOD_SECS,
        recipient: ADDRS.VAULTS.SUSDSpp.MANAGER,
        appData: DEFAULT_SETTINGS.VAULTS.SUSDSpp.COW_SWAPPERS.EXACT_SKY_TO_USDS.APP_DATA,
      }
    )
  );

  await mine(
    INSTANCES.VAULTS.SUSDSpp.COW_SWAPPER.setCowApproval(
      ADDRS.EXTERNAL.SKY.SKY_TOKEN, 
  ...
  await mine(
    INSTANCES.VAULTS.SUSDSpp.COW_SWAPPER.createConditionalOrder(ADDRS.EXTERNAL.SKY.SKY_TOKEN)
  );
}
...
async function main() {
  ...
  await mine(
    INSTANCES.VAULTS.SUSDSpp.TOKEN.setManager(ADDRS.VAULTS.SUSDSpp.MANAGER),
  );

  await setupCowSwapper();

  await mine(
    INSTANCES.VAULTS.SUSDSpp.MANAGER.addFarm(ADDRS.EXTERNAL.SKY.STAKING_FARMS.USDS_SKY, 111)
  );
  await mine(
    INSTANCES.VAULTS.SUSDSpp.MANAGER.addFarm(ADDRS.EXTERNAL.SKY.STAKING_FARMS.USDS_SDAO, 222)
  );
