...
import {
  ...
  DummyDelegated4626VaultManager, DummyDelegated4626VaultManager__factory,
...
export function connectToContracts1(owner: Signer, ADDRS: ContractAddresses): ContractInstances {
  return {
    ...
    VAULTS: {
      SUSDSpp: {
        TOKEN: OrigamiSuperSavingsUsdsVault__factory.connect(ADDRS.VAULTS.SUSDSpp.TOKEN, owner),
        MANAGER: OrigamiSuperSavingsUsdsManager__factory.connect(ADDRS.VAULTS.SUSDSpp.MANAGER, owner),
        COW_SWAPPER: OrigamiCowSwapper__factory.connect(ADDRS.VAULTS.SUSDSpp.COW_SWAPPER, owner),
      },
    },
    EXTERNAL: {
      SKY: {
