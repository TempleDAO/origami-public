...
contract OrigamiOracleWithPriceCheck is OrigamiElevatedAccess, IOrigamiOracle {
    ...
    constructor(address _initialOwner, address _underlyingOracle, Range.Data memory _validPriceRange)
     ...
     */
    function setValidPriceRange(uint128 _validPriceFloor, uint128 _validPriceCeiling) external onlyElevatedAccess {
        ...
        returns (uint256 price)
    {
        ...
        if (price < _validPriceRange.floor) {
            revert BelowMinValidRange(address(underlyingOracle), price, _validPriceRange.floor);
        }
        if (price > _validPriceRange.ceiling) {
            revert AboveMaxValidRange(address(underlyingOracle), price, _validPriceRange.ceiling);
        }
    }
    ...
    ) external view override returns (uint256 toAssetAmount) {
        if (fromAsset == underlyingOracle.baseAsset()) {
            // The numerator needs to round in the same way to be conservative
            uint256 _price = latestPrice(priceType, roundingMode);

            return fromAssetAmount.mulDiv(_price, underlyingOracle.assetScalingFactor(), roundingMode);
            ...
            uint256 _price = latestPrice(
                priceType,
                roundingMode == OrigamiMath.Rounding.ROUND_UP
                    ? OrigamiMath.Rounding.ROUND_DOWN
                    : OrigamiMath.Rounding.ROUND_UP
            );

            if (_price == 0) revert InvalidPrice(address(this), int256(_price));
            return fromAssetAmount.mulDiv(underlyingOracle.assetScalingFactor(), _price, roundingMode);
        }
