...
contract OrigamiScaledOracle is OrigamiOracleBase {
    ...
    constructor(BaseOracleParams memory baseParams, address _referenceOracle, address _scalarOracle, bool _multiply)
        OrigamiOracleBase(baseParams)
    {
        ...
        if (scalarOracle.baseAsset() != address(0) || scalarOracle.quoteAsset() != address(0)) {
        ...
        returns (uint256)
    {
        ...
        uint256 _referencePrice = referenceOracle.latestPrice(priceType, roundingMode);

        if (multiply) {
            // Also the numerator - so follow the requested roundingMode
            uint256 _scalarPrice = scalarOracle.latestPrice(priceType, roundingMode);
