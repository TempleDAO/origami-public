 ...
 */
contract OrigamiDelegated4626Vault is OrigamiErc4626, IOrigamiDelegated4626Vault {
    ...
    constructor(address initialOwner_, string memory name_, string memory symbol_, IERC20 asset_, address tokenPrices_)
    ...
    function setManager(address newManagerAddress, uint256 minNewAssets) external virtual override onlyElevatedAccess {
        ...
        if (address(newManager.vault()) != address(this)) {
            revert CommonEventsAndErrors.InvalidAddress(newManagerAddress);
        }
        ...
        if (address(oldManager) != address(0)) {
            ...
            // withdrawn just-in-time, this is expected to fail. In that case, the assets must be made available to
            // withdraw prior to migration.
            ...
            // It may be slightly less than initially due to underlying rounding. Elevated access will use a
            // reasonable/small slippage to calculate this offchain first.
    ...
    function areDepositsPaused() public view virtual override(IOrigamiErc4626, OrigamiErc4626) returns (bool) {
        return _manager.areDepositsPaused();
    }

    /// @inheritdoc IOrigamiErc4626
    function areWithdrawalsPaused() public view virtual override(IOrigamiErc4626, OrigamiErc4626) returns (bool) {
        return _manager.areWithdrawalsPaused();
    }

    /// @inheritdoc IOrigamiDelegated4626Vault
    function manager() external view virtual override returns (address) {
        return address(_manager);
    }

    /// @inheritdoc IOrigamiDelegated4626Vault
    function performanceFeeBps() external view virtual override returns (uint48) {
    ...
    function _withdrawHook(uint256 assets, address receiver) internal virtual override {
    ...
    function maxDeposit(
        ...
        public
        view
        override(IERC4626, OrigamiErc4626)
        ...
        returns (uint256 maxShares)
    {
        ...
        (uint256 maxSharesFromManager,) = _previewDeposit(maxAssetsfromManager, depositFeeBps());

        return fromTotalSupplyCap < maxSharesFromManager ? fromTotalSupplyCap : maxSharesFromManager;
    }

    /// @inheritdoc IERC4626
    function maxWithdraw(address sharesOwner)
        public
        view
        override(IERC4626, OrigamiErc4626)
        returns (uint256 maxAssets)
    ...
    /// @inheritdoc IERC4626
    function maxRedeem(address sharesOwner) public view override(IERC4626, OrigamiErc4626) returns (uint256 maxShares) {
        uint256 fromTotalSupplyCap = _maxRedeem(sharesOwner);
        uint256 maxAssetsfromManager = _manager.maxWithdraw();
        if (maxAssetsfromManager == type(uint256).max) return fromTotalSupplyCap;

        (uint256 maxSharesFromManager,) = _previewWithdraw(maxAssetsfromManager, withdrawalFeeBps());
