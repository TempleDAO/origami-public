...
contract OrigamiHOhmArbBot is IOrigamiHOhmArbBot, IMorphoFlashLoanCallback, OrigamiElevatedAccess {
    ...
    /// @inheritdoc IOrigamiHOhmArbBot
    function approveToken(IERC20 token, address spender, uint256 amount) external override onlyElevatedAccess {
        token.forceApprove(spender, amount);
    }

    /// @inheritdoc IOrigamiHOhmArbBot
    function recoverToken(IERC20 token, address to, uint256 amount) external override onlyElevatedAccess {
        // The asset token is sent straight to the manager on deposit/withdraw - so it's acceptable to recover from
        // here.
    ...
    function quoteRoute1(uint256 sUsdsSold, uint24 susdsHohmPoolFee, uint24 ohmSusdsPoolFee)
        external
        override
        returns (Route1Quote memory quoteData)
    {
        // Sell sUSDS to buy hOHM on uniswap
        uint256 hohmBought = _uniV3Quote(sUSDS, sUsdsSold, hOHM, susdsHohmPoolFee);
        ...
        uint256 sUsdsRevenue = _uniV3Quote(OHM, quoteData.ohmSold, sUSDS, ohmSusdsPoolFee);
    ...
    function _handleRoute1Flashloan(_FlashloanData memory data) private {
        // Sell sUSDS to buy hOHM on uniswap
        uint256 hohmBought = _uniV3Swap(sUSDS, data.sUsdsSold, hOHM, data.susdsHohmPoolFee, data.deadline);
        ...
        _uniV3Swap(OHM, ohmFromHohmExit, sUSDS, data.ohmSusdsPoolFee, data.deadline);
    }
        ...
        returns (Route2Quote memory quoteData)
    {
        ...
        quoteData.ohmBought = _uniV3Quote(sUSDS, sUsdsSold, OHM, ohmSusdsPoolFee);

        quoteData.gOhmReceived = gOHM.balanceTo(quoteData.ohmBought);

        // Mint hOHM with the gOHM, also receiving USDS liabilities
        (quoteData.hOhmMinted, quoteData.usdsReceived) = _mintHohmQuote(quoteData.gOhmReceived);

        // Mint sUSDS with the USDS
        uint256 sUsdsReceived = sUSDS.previewDeposit(quoteData.usdsReceived);

        // Sell hOHM for sUSDS
        uint256 sUsdsBought = _uniV3Quote(hOHM, quoteData.hOhmMinted, sUSDS, susdsHohmPoolFee);
    ...
    function _handleRoute2Flashloan(_FlashloanData memory data) private {
        // Sell sUSDS to buy OHM on uniswap
        uint256 ohmBought = _uniV3Swap(sUSDS, data.sUsdsSold, OHM, data.ohmSusdsPoolFee, data.deadline);

        // Mint hOHM with the gOHM, also receiving USDS liabilities
        uint256 gohmBought = olympusStaking.stake(address(this), ohmBought, false, true);
        (uint256 hohmMinted, uint256 usdsReceived) = _mintHohm(gohmBought);

        // Mint sUSDS with the USDS
        sUSDS.deposit(usdsReceived, address(this));

        // Sell hOHM for sUSDS
        _uniV3Swap(hOHM, hohmMinted, sUSDS, data.susdsHohmPoolFee, data.deadline);
    }
    ...
    function _mintHohmQuote(uint256 gOhmAmountIn) internal view returns (uint256 hOhmSharesOut, uint256 usdsAmountOut) {
        uint256[] memory liabilities;
        (hOhmSharesOut,/*assets*/, liabilities) = hOHM.previewJoinWithToken(address(gOHM), gOhmAmountIn);
        usdsAmountOut = liabilities[0];
    }

    function _mintHohm(uint256 gOhmAmountIn) internal returns (uint256 hOhmSharesOut, uint256 usdsAmountOut) {
        uint256[] memory liabilities;
        (hOhmSharesOut,/*assets*/, liabilities) =
            hOHM.joinWithToken(address(gOHM), gOhmAmountIn, address(this), hohmTokensHash);
        usdsAmountOut = liabilities[0];
    }

    function _exitHohmQuote(uint256 hOhmAmountIn) internal view returns (uint256 gOhmAmountOut, uint256 usdsAmountIn) {
        (uint256[] memory assets, uint256[] memory liabilities) = hOHM.previewExitWithShares(hOhmAmountIn);
        gOhmAmountOut = assets[0];
        usdsAmountIn = liabilities[0];
    }

    function _exitHohm(uint256 hohmAmountOut) internal returns (uint256 gOhmAmountOut, uint256 usdsAmountIn) {
        (uint256[] memory assets, uint256[] memory liabilities) =
            hOHM.exitWithShares(hohmAmountOut, address(this), address(this), hohmTokensHash);
        gOhmAmountOut = assets[0];
        usdsAmountIn = liabilities[0];
    }

    function _uniV3Quote(IERC20 tokenIn, uint256 amountIn, IERC20 tokenOut, uint24 fee)
        private
        returns (uint256 amountOut)
    {
        IUniswapV3QuoterV2.QuoteExactInputSingleParams memory quoteParams =
        ...
        (
            amountOut,/*uint160 sqrtPriceX96After*/,/*uint32 initializedTicksCrossed*/,
            /*uint256 gasEstimate*/
        ) = uniV3Quoter.quoteExactInputSingle(quoteParams);
    }

    function _uniV3Swap(IERC20 tokenIn, uint256 amountIn, IERC20 tokenOut, uint24 fee, uint256 deadline)
        internal
        returns (uint256 amountOut)
    {
