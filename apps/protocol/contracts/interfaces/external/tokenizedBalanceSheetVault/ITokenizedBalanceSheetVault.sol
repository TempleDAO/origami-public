 ...
 * @notice
 *  Shares in this vault represent a proportional slice of the balance sheet of this vault. The balance sheet adheres to
 * the
 ...
 *    - They PROVIDE a proportional amount of each ASSET in the balance sheet as of that moment, for that number of
 * shares.
 *    - They RECEIVE a proportional amount of each LIABILITY in the balance sheet as of that moment, for that number of
 * shares.
 *    - The shares representing that proportional slice of the balance sheet equity are minted.
 *
 *  When a user redeems shares:
 *    - They PROVIDE a proportional amount of each LIABILITY in the balance sheet as of that moment, for that number of
 * shares.
 *    - They RECEIVE a proportional amount of each ASSET in the balance sheet as of that moment, for that number of
 * shares.
 ...
 *    - To ensure the caller joining/exiting is using the required token addresses as of the latest block, a bytes32
 * hash of
 ...
 *        Caller specifies the number of shares to mint and the amount of each ASSET (pulled from user) and LIABILITY
 * (sent to user)
 *        token is derived from the existing balance weights of the vault's balance sheet.
 *    - joinWithToken(tokenAddress, amount):
 *        Caller specifies either one of the ASSET addresses (pulled from user) or one of the LIABILITY addresses (sent
 * to user),
 *        and the amount of that token. The number of shares to mint, and the required number of the remaining ASSET
 * (pulled from user) and
 ...
 *        Caller specifies the number of shares to burn and the amount of each ASSET (sent to user) and LIABILITY
 * (pulled from user)
 *        token is derived from the existing balance weights of the vault's balance sheet.
 *    - exitWithToken(tokenAddress, amount):
 *        Caller specifies either one of the ASSET addresses (sent to user) or one of the LIABILITY addresses (pulled
 * from user),
 *        and the amount of that token. The number of shares to burn, and the required number of the remaining ASSET
 * (sent to user) and
 *        LIABILITY (pulled from user) tokens are derived from the existing balance weights of the vault's balance
 * sheet.
 *
 ...
 *    - There is no realisation of exposure (from a vault perspective) from one asset/liability into a single vault
 * asset at a certain point in time.
 *    - The vault is not affected if there is a lack of liquidity to convert the other assets/liabilities into a single
 * vault asset.
 *
 * A drawback is that the UX and integration is obviously tricker.
 *    - In most cases, it's likely that there will be AMM liquidity in order for users to easily buy/sell the vault
 * token, rather than minting redeeming
 ...
 *    - 'zaps' can be added where possible for dapps to allocate in to the right assets - eg to provide the best price
 * to users (AMM buy vs direct mint)
 *    - The asset and liability tokens can change over time depending on the specific implementation. Integrators need
 * to be aware of this.
 *
 * As with ERC-4626, there is no slippage/deadline guarantees within this interface enforcing bounds on the
 * sent/received. If required for a particular integration,
 * it can be handled via an intermediate 'router' contract enforcing bounds on the assets/liabilities/shares which are
 * transferred.
 */
interface ITokenizedBalanceSheetVault is IERC20, IERC20Metadata {
    ...
    function exitWithToken(
        ...
        address owner,
        bytes32 tokensHash
    ) external returns (uint256 shares, uint256[] memory assets, uint256[] memory liabilities);
     ...
     * NOTE: most implementations will require pre-approval of the Vault with all of the liability tokens.
     */
    function exitWithShares(uint256 shares, address receiver, address owner, bytes32 tokensHash)
        external
        returns (uint256[] memory assets, uint256[] memory liabilities);
}
