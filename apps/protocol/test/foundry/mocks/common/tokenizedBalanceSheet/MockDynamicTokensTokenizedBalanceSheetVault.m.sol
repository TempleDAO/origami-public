...
contract MockDynamicTokensTokenizedBalanceSheetVault is MockTokenizedBalanceSheetVaultWithFees {
    ...
    constructor(
        ...
        MockTokenizedBalanceSheetVaultWithFees(
            initialOwner_, name_, symbol_, assetTokens_, liabilityTokens_, joinFeeBps_, exitFeeBps_, borrowLend_
        )
    { }
    ...
    function rebalance(Rollover calldata tokensRollover) external {
        ...
        for (uint256 i; i < tokensRollover.tokensExpired.length; i++) {
            MintableToken(tokensRollover.tokensRenewed[i]).approve(address(borrowLend), type(uint256).max); //note:
            // normally we should do this in a separate function, but for sake of the mocking
    ...
    function _mockSwapExactTokensForTokens(address tokenIn, address tokenOut, uint256 amountIn, uint256 amountOutMin)
        internal
        returns (uint256)
    {
