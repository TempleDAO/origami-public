...
abstract contract TokenizedBalanceSheetProp is Test {
    ...
    bytes32 internal _vaultTokensHash;

    bool internal _vaultMayBeEmpty;
    bool internal _unlimitedAmount;

    function _assets() internal view returns (address[] memory assets) {
        (assets,) = _vault_.tokens();
    }

    function _liabilities() internal view returns (address[] memory liabilities) {
        (, liabilities) = _vault_.tokens();
    }

    function _updateTokensHash() internal {
        _vaultTokensHash = _vault_.currentTokensHash();
    }

    // tokens
    // "MUST NOT revert."
    function prop_tokens(
        address /*caller*/
    )
        public
        view
    ...
    function prop_convertFromToken(address caller1, address caller2, address token, uint256 amount) public {
        ...
        (uint256 shares1, uint256[] memory assets1, uint256[] memory liabilities1) =
            vault_convertFromToken(token, amount);
        vm.prank(caller2); // "MAY revert due to integer overflow caused by an unreasonably large input."
        (uint256 shares2, uint256[] memory assets2, uint256[] memory liabilities2) =
            vault_convertFromToken(token, amount);
    ...
    function prop_convertFromShares(address caller1, address caller2, uint256 shares) public {
        vm.prank(caller1); // "MAY revert due to integer overflow caused by an unreasonably large input."
        (uint256[] memory assets1, uint256[] memory liabilities1) = vault_convertFromShares(shares);
        vm.prank(caller2); // "MAY revert due to integer overflow caused by an unreasonably large input."
        (uint256[] memory assets2, uint256[] memory liabilities2) = vault_convertFromShares(shares);
    ...
    function prop_previewJoinWithToken(
        address token,
        address caller,
        address receiver,
        address other,
        ...
        (uint256 sharesPreview,,) = vault_previewJoinWithToken(token, tokenAmount);
        vm.assume(sharesPreview > 0);
        vm.prank(caller);
        (uint256 sharesActual,,) = vault_joinWithToken(token, tokenAmount, receiver);
    ...
    function prop_joinWithToken(address token, address caller, address receiver, uint256 tokenAmount) public {
        ...
        vm.assume(pshares > 0);
        vm.prank(caller);
        (uint256 shares, uint256[] memory assetsJoin, uint256[] memory liabilitiesJoin) =
            vault_joinWithToken(token, tokenAmount, receiver);
    ...
    function prop_previewJoinWithShares(address caller, address receiver, address other, uint256 shares) public {
        vm.prank(other);
        (uint256[] memory assets1, uint256[] memory liabilities1) = vault_previewJoinWithShares(shares);
        vm.prank(caller);
        (uint256[] memory assets2, uint256[] memory liabilities2) = vault_joinWithShares(shares, receiver);
    ...
    function prop_joinWithShares(address caller, address receiver, uint256 shares) public {
        ...
        vm.prank(caller);
        (uint256[] memory assetAmounts, uint256[] memory liabilityAmounts) = vault_joinWithShares(shares, receiver);

        for (uint8 i; i < aLength; i++) {
            assertApproxEqAbs(
                IERC20(assets[i]).balanceOf(caller), oldBalancesAssets[i] - assetAmounts[i], _delta_, "asset"
            );
        }

        for (uint8 i; i < lLength; i++) {
            assertApproxEqAbs(
                IERC20(liabilities[i]).balanceOf(receiver),
                oldBalancesLiabilities[i] + liabilityAmounts[i],
                _delta_,
                "liabilities"
            );
        }
    ...
    function prop_previewExitWithToken(
        address token,
        address caller,
        address receiver,
        address owner,
        ...
        vm.prank(other);
        (uint256 sharesPreview,,) = vault_previewExitWithToken(token, tokenAmount);
        vm.assume(sharesPreview > 0);
        vm.prank(caller);
        (uint256 sharesActual,,) = vault_exitWithToken(token, tokenAmount, receiver, owner);
        assertApproxLeAbs(sharesActual, sharesPreview, _delta_);
    }

    // exitWithToken
    function prop_exitWithToken(
        IERC20 token,
        bool isAsset,
        address caller,
        address receiver,
        ...
        vm.assume(pshares > 0);
        vm.prank(caller);
        (uint256 shares,,) = vault_exitWithToken(address(token), tokenAmount, receiver, owner);
    ...
    function prop_previewExitWithShares(address caller, address receiver, address owner, address other, uint256 shares)
        public
    {
        vm.prank(other);
        (uint256[] memory assetsPreview, uint256[] memory liabilitiesPreview) = vault_previewExitWithShares(shares);
        vm.prank(caller);
        (uint256[] memory assetsJoin, uint256[] memory liabilitiesJoin) = vault_exitWithShares(shares, receiver, owner);
    ...
    function prop_exitWithShares(address caller, address receiver, address owner, uint256 shares) public {
        ...
        vm.prank(caller);
        (uint256[] memory assetsExit, uint256[] memory liabilitiesExit) = vault_exitWithShares(shares, receiver, owner);

        for (uint256 i; i < assets.length; i++) {
            uint256 bal = IERC20(assets[i]).balanceOf(receiver);
            assertApproxEqAbs(bal, oldBalancesAssets[i] + assetsExit[i], _delta_, "asset"); // NOTE: this may fail if
            // the receiver is a contract in which the asset is stored
        }

        for (uint256 i; i < liabilities.length; i++) {
            uint256 bal = IERC20(liabilities[i]).balanceOf(caller);
            assertApproxEqAbs(bal, oldBalancesLiabilities[i] - liabilitiesExit[i], _delta_, "liabilities"); // NOTE:
            // this may fail if the receiver is a contract in which the asset is stored
        }
    ...
    function prop_RT_previewJoinWithToken(address token, bool isAsset, uint256 tokenIndex, uint256 tokenAmount)
        public
        view
    {
        (uint256 shares, uint256[] memory assetsJoin, uint256[] memory liabilitiesJoin) =
            vault_previewJoinWithToken(token, tokenAmount);
        vm.assume(shares > 0);
    ...
    function prop_RT_previewExitWithToken(address token, bool isAsset, uint256 tokenIndex, uint256 tokenAmount)
        public
        view
    {
        (, uint256[] memory assetsExit, uint256[] memory liabilitiesExit) =
            vault_previewExitWithToken(token, tokenAmount);
    ...
    function prop_RT_joinWithToken_exitWithShares(address token, address caller, uint256 tokenAmount) public {
        ...
        vm.prank(caller);
        (uint256 shares, uint256[] memory assetsJoin, uint256[] memory liabilitiesJoin) =
            vault_joinWithToken(token, tokenAmount, caller);
        vm.prank(caller);
        (uint256[] memory assetsExit, uint256[] memory liabilitiesExit) = vault_exitWithShares(shares, caller, caller);
    ...
    // s' = exitWithToken(a)
    // s' >= s
    function prop_RT_joinWithToken_exitWithToken(
        address token,
        bool isAsset,
        uint256 tokenIndex,
        address caller,
        ...
        vm.assume(shares > 0);
        vm.prank(caller);
        (uint256 sharesJoin, uint256[] memory assetsJoin, uint256[] memory liabilitiesJoin) =
            vault_joinWithToken(token, tokenAmount, caller);
        ...
        vm.assume(shares > 0);
        vm.prank(caller);
        (uint256 sharesExit, uint256[] memory assetsExit, uint256[] memory liabilitiesExit) =
            vault_exitWithToken(token, amount < maxExit ? amount : maxExit, caller, caller);
    ...
    //1. rounding the assets down and then passing 0 to the join, this will break the assertion but works correctly,
    // since exit must round down. 2. passing liability to the join will cause the shares assertion to break since
    // liabilities are rounded up when exiting, therefore the shares from the join are more than when exiting.
    function prop_RT_exitWithShares_joinWithToken(
        address token,
        bool isAsset,
        uint256 tokenIndex,
        address caller,
        ...
        vm.assume(pshares > 0);
        vm.prank(caller);
        (uint256 sharesJoin,,) = vault_joinWithToken(token, amount, caller);
    ...
    function prop_RT_exitWithShares_joinWithShares(address caller, uint256 shares) public {
        ...
        if (!_vaultMayBeEmpty) vm.assume(_vault_.totalSupply() > 0);
        vm.prank(caller);
        (uint256[] memory assetsJoin, uint256[] memory liabilitiesJoin) = vault_joinWithShares(shares, caller);
    ...
    function prop_RT_joinWithShares_exitWithToken(
        address token,
        bool isAsset,
        uint256 tokenIndex,
        address caller,
        ...
        vm.assume(pshares > 0);
        vm.prank(caller);
        (uint256 shares2,,) = vault_exitWithToken(token, amount, caller, caller);
        assertApproxGeAbs(shares2, shares, _delta_ + 0.005e18); //note Depending on which token is passed and the ratio
        // some roundings happen which causes join to give more tokens than exit
    }
    ...
    function prop_RT_exitWithToken_joinWithShares(address token, address caller, uint256 tokenAmount) public {
        ...
        vm.assume(pshares > 0);
        vm.prank(caller);
        (uint256 shares, uint256[] memory assetsExit, uint256[] memory liabilitiesExit) =
            vault_exitWithToken(token, tokenAmount, caller, caller);
        if (!_vaultMayBeEmpty) vm.assume(_vault_.totalSupply() > 0);
    ...
    // s' = joinWithToken(a)
    // s' <= s
    function prop_RT_exitWithToken_joinWithToken(bool isAsset, address token, address caller, uint256 tokenAmount)
        public
    {
        (uint256 shares,,) = _vault_.previewExitWithToken(token, tokenAmount);
        vm.assume(shares > 0);
        vm.prank(caller);
        (uint256 sharesFromExit, uint256[] memory assetsExit, uint256[] memory liabilitiesExit) =
            vault_exitWithToken(token, tokenAmount, caller, caller);
        if (!_vaultMayBeEmpty) vm.assume(_vault_.totalSupply() > 0);
        (shares,,) = _vault_.previewJoinWithToken(token, tokenAmount);
        vm.assume(shares > 0);
        vm.prank(caller);
        (uint256 sharesFromJoin, uint256[] memory assetsJoin, uint256[] memory liabilitiesJoin) =
            vault_joinWithToken(token, tokenAmount, caller);
    ...
    function prop_RT_round_trips_tokens(
        ...
        (previewJoinTemplate.shares, previewJoinTemplate.assetAmounts, previewJoinTemplate.liabilityAmounts) =
            vault_previewJoinWithToken(token, amount);

        PreviewData memory preivewData;
        preivewData.shares = previewJoinTemplate.shares;
        (preivewData.assetAmounts, preivewData.liabilityAmounts) =
            vault_previewJoinWithShares(previewJoinTemplate.shares);
        ...
        if (randAmount > 1e3) {
            (preivewData.shares, preivewData.assetAmounts, preivewData.liabilityAmounts) =
                vault_previewJoinWithToken(assets[randAssetIndex], randAmount);
            checkPreviewJoin(previewJoinTemplate, preivewData);
        }
        ...
        if (randAmount > 1e3) {
            (preivewData.shares, preivewData.assetAmounts, preivewData.liabilityAmounts) =
                vault_previewJoinWithToken(liabilities[randLiabilityIndex], randAmount);
            checkPreviewJoin(previewJoinTemplate, preivewData);
        }
    }
    ...
    function vault_convertFromToken(address token, uint256 amount)
        internal
        view
        returns (uint256 sharesConvert, uint256[] memory assetsConver, uint256[] memory liabilitiesConvert)
    {
        return _vault_.convertFromToken(token, amount);
    }

    function vault_convertFromShares(uint256 shares)
        internal
        view
        returns (uint256[] memory assetsConvert, uint256[] memory liabilitiesConvert)
    {
        return _vault_.convertFromShares(shares);
    }

    function vault_previewJoinWithToken(address token, uint256 amount)
        internal
        view
        returns (uint256 sharesPreview, uint256[] memory assetsPreview, uint256[] memory liabilitiesPreview)
    {
        return _vault_.previewJoinWithToken(token, amount);
    }

    function vault_previewJoinWithShares(uint256 shares)
        internal
        view
        returns (uint256[] memory assetsPreview, uint256[] memory liabilitiesPreview)
    {
        return _vault_.previewJoinWithShares(shares);
    }

    function vault_previewExitWithToken(address token, uint256 amount)
        internal
        view
        returns (uint256 sharesPreview, uint256[] memory assetsPreview, uint256[] memory liabilitiesPreview)
    {
    ...
    function vault_previewExitWithShares(uint256 shares)
        internal
        view
        returns (uint256[] memory assetsPreview, uint256[] memory liabilitiesPreview)
    {
    ...
    function vault_joinWithToken(address token, uint256 amount, address receiver)
        internal
        returns (uint256 sharesJoin, uint256[] memory assetsJoin, uint256[] memory liabilitiesJoin)
    {
        return _vault_.joinWithToken(token, amount, receiver, _vaultTokensHash);
    }

    function vault_joinWithShares(uint256 shares, address receiver)
        internal
        returns (uint256[] memory assetsJoin, uint256[] memory liabilitiesJoin)
    {
        return _vault_.joinWithShares(shares, receiver, _vaultTokensHash);
    }

    function vault_exitWithToken(address token, uint256 amount, address receiver, address owner)
        internal
        returns (uint256 sharesExit, uint256[] memory assetsExit, uint256[] memory liabilitiesExit)
    {
        return _vault_.exitWithToken(token, amount, receiver, owner, _vaultTokensHash);
    }

    function vault_exitWithShares(uint256 shares, address receiver, address owner)
        internal
        returns (uint256[] memory assetsExit, uint256[] memory liabilitiesExit)
    {
        return _vault_.exitWithShares(shares, receiver, owner, _vaultTokensHash);
    }
