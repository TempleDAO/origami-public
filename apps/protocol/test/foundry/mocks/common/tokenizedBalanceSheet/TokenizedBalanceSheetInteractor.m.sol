...
contract Interactor is CommonBase, StdUtils, StdCheats {
    ...
    event InteractorExitToken(
        address token, uint256[] assets, uint256[] liabilities, uint256 shares, address receiver, address sharesOwner
    );
    event InteractorExitShares(
        uint256[] assets, uint256[] liabilities, uint256 shares, address receiver, address sharesOwner
    );

    MockTokenizedBalanceSheetVaultWithFees private immutable vault;
    MockBorrowLend private immutable borrowLend;
    bytes32 private immutable tokenHash;
    ...
    function joinToken(bool isAsset, uint8 tokenIndex, uint256 amount) external {
        ...
        if (isAsset) {
            ...
            amount = _bound(amount, 0, balanceOfThis < 100_000e18 ? balanceOfThis : 100_000_000e18);
        } else {
        ...
        (uint256 shares, uint256[] memory assetsGiven, uint256[] memory liabReceived) =
            vault.previewJoinWithToken(_token, amount);
        vm.assume(shares > 0);

        _doJoin(assetsGiven, liabReceived, shares);

        vault.joinWithToken(_token, amount, address(this), tokenHash);
    ...
    function joinShares(uint256 shares) external {
        ...
        (uint256[] memory assetsGiven, uint256[] memory liabReceived) = vault.previewJoinWithShares(shares);

        _doJoin(assetsGiven, liabReceived, shares);

        vault.joinWithShares(shares, address(this), tokenHash);
    ...
    function exitToken(bool isAsset, uint8 tokenIndex, uint256 amount) external {
        ...
        (uint256 shares, uint256[] memory assetsReceived, uint256[] memory liabilitiesGiven) =
        ...
        vault.exitWithToken(_token, amount, address(this), address(this), tokenHash);
    ...
    function exitShares(uint256 shares) external {
        ...
        (uint256[] memory assetsReceived, uint256[] memory liabilitiesGiven) = vault.previewExitWithShares(shares);

        _doExit(assetsReceived, liabilitiesGiven, shares);
        vault.exitWithShares(shares, address(this), address(this), tokenHash);
    ...
    function increaseSharePrice(
        ...
        asset1Amount = _bound(
            asset1Amount,
            0,
            IERC20(assetsCached.at(0)).balanceOf(address(this)) > 100_000e18
                ? 100_000e18
                : IERC20(assetsCached.at(0)).balanceOf(address(this))
        );
        asset2Amount = _bound(
            asset2Amount,
            0,
            IERC20(assetsCached.at(1)).balanceOf(address(this)) > 100_000e6
                ? 100_000e6
                : IERC20(assetsCached.at(1)).balanceOf(address(this))
        );

        debt1Amount = _bound(
            debt1Amount,
            0,
            IERC20(liabilitiesCached.at(0)).balanceOf(address(borrowLend)) > 100_000e18
                ? 100_000e18
                : IERC20(liabilitiesCached.at(0)).balanceOf(address(borrowLend))
        );
        debt2Amount = _bound(
            debt2Amount,
            0,
            IERC20(liabilitiesCached.at(1)).balanceOf(address(borrowLend)) > 100_000e6
                ? 100_000e6
                : IERC20(liabilitiesCached.at(1)).balanceOf(address(borrowLend))
        );
