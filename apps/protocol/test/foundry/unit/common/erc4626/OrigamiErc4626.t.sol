...
contract OrigamiErc4626TestBase is OrigamiTest {
    ...
    function deposit(address user, uint256 amount) internal {
        ...
        if (expectedFeeAmout > 0) {
            ...
            emit InKindFees(IOrigamiErc4626.FeeType.DEPOSIT_FEE, DEPOSIT_FEE, expectedFeeAmout);
        }
...
contract OrigamiErc4626TestAdmin is OrigamiErc4626TestBase {
    ...
    function test_default_nofees() public {
        asset = new DummyMintableToken(origamiMultisig, "UNDERLYING", "UDLY", 18);
        vault = MockErc4626VaultWithFees(address(new OrigamiErc4626(origamiMultisig, "VAULT", "VLT", asset)));
    ...
    function test_setMaxTotalSupply_failure_zeroSupply() public {
        vault = new MockErc4626VaultWithFees(origamiMultisig, "VAULT", "VLT", asset, 0, 0);
        vm.startPrank(origamiMultisig);
    ...
    function test_seedDeposit_failure_maxTooLow() public {
        ...
        vm.startPrank(origamiMultisig);
        vm.expectRevert(
            abi.encodeWithSelector(IOrigamiErc4626.ERC4626ExceededMaxDeposit.selector, origamiMultisig, 123e18, 1e18)
        );
...
contract OrigamiErc4626TestMint is OrigamiErc4626TestBase {
    ...
    function test_mint_beforeShareIncrease() public {
        ...
        assertEq(vault.totalSupply(), 123e18 + 0.0995e18);
        assertEq(vault.totalAssets(), expectedAssets);
        assertEq(vault.convertToShares(1e18), 0.54872020727324886e18);
    ...
    function test_mint_afterShareIncrease() public {
        ...
        assertEq(vault.maxDeposit(alice), 111_554_770.93887930594493214e18);
...
contract OrigamiErc4626TestWithdraw is OrigamiErc4626TestBase {
    ...
    function test_withdraw_afterShareIncrease() public {
        ...
        assertEq(vault.convertToAssets(1e18), 1.13016508335355482e18);
    }
...
contract OrigamiErc4626TestRedeem is OrigamiErc4626TestBase {
    ...
    function test_redeem_ok_zeroShares() public {
        vault = new MockErc4626VaultWithFees(origamiMultisig, "VAULT", "VLT", asset, 0, WITHDRAWAL_FEE);
        seedDeposit(origamiMultisig, SEED_DEPOSIT_SIZE, MAX_TOTAL_SUPPLY);
...
contract OrigamiErc4626TestPermit is OrigamiErc4626TestBase {
    ...
    function test_permit() public {
        ...
        vm.expectRevert(abi.encodeWithSelector(IOrigamiErc4626.ERC2612ExpiredSignature.selector, 99_999_999));
        ...
        vm.expectRevert(
            abi.encodeWithSelector(IOrigamiErc4626.ERC2612InvalidSigner.selector, wrongRecoveryAddr, signer)
        );
...
contract OrigamiErc4626TestAttacksSB is OrigamiErc4626TestBase {
    function test_erc4626_failedInflationAttack_fees() public {
        ...
        {
            deal(address(asset), bob, 10_000e18);
            vm.prank(bob);
        ...
        {
            vm.startPrank(bob);
            assertEq(vault.redeem(vault.balanceOf(bob), bob, bob), 100_755);
...
contract OrigamiErc4626TestAttacksJP is OrigamiErc4626TestBase {
    function test_erc4626_donationAttack_withFees() public {
        ...
        deal(address(asset), alice, 10_000e18);
        ...
        vm.prank(attacker);
        asset.transfer(address(vault), 10_000e18);
        assertEq(vault.convertToAssets(1), 100_503);
        assertEq(vault.totalSupply(), 0.0995e18 + 1);
        ...
        assertEq(vault.deposit(15_000e18, attacker), 0.148131011171175883e18);
        ...
        assertEq(vault.maxWithdraw(alice), 9796.43076565532528139e18);
        assertEq(vault.redeem(vault.maxRedeem(alice), alice, alice), 9796.43076565532528139e18);
        ...
        vm.startPrank(origamiMultisig);
        assertEq(
            vault.redeem(vault.maxRedeem(origamiMultisig), origamiMultisig, origamiMultisig),
        ...
        assertEq(asset.balanceOf(attacker), 14_775.112743048338451082e18); // down bad
        assertEq(asset.balanceOf(alice), 9796.43076565532528139e18); // down a bit from fees
    ...
    function test_erc4626_donationAttack_noFees() public {
        ...
        assertEq(vault.maxWithdraw(attacker), 100_000);

        vm.startPrank(attacker);
        assertEq(vault.redeem(vault.maxRedeem(attacker), attacker, attacker), 100_000);
        assertEq(vault.totalSupply(), 0.1e18 + 0.02999970000299997e18);
        assertEq(vault.balanceOf(alice), 0.02999970000299997e18);
        assertEq(vault.balanceOf(attacker), 0);
