...
import {
    MockDynamicTokensTokenizedBalanceSheetVault
} from "test/foundry/mocks/common/tokenizedBalanceSheet/MockDynamicTokensTokenizedBalanceSheetVault.m.sol";
import { MockBorrowLend } from "test/foundry/mocks/common/tokenizedBalanceSheet/MockBorrowLend.m.sol";

//TODO: combine all the common functionalities into one contract, just make sure to give freedom on the vault
// implementation TODO: rename to DynamicTokensOrigamiTokenizedBalanceSheetVaultTestBASE
contract DynamicTokensOrigamiTokenizedBalanceSheetVaultTestBase is OrigamiTest {
    ...
    bytes32 internal _dynamicVaultTokensHash;
    ...
    function setUpVault(uint16 joinFeeBps, uint16 exitFeeBps) internal {
        ...
        vault = new MockDynamicTokensTokenizedBalanceSheetVault(
            origamiMultisig,
        ...
        _dynamicVaultTokensHash = vault.currentTokensHash();
        vm.warp(100_000_000);
    ...
    function joinWithToken(address user, address receiver, IERC20 token, uint256 tokenAmount) internal {
        ...
        (uint256 actualShares, uint256[] memory actualAssets, uint256[] memory actualLiabilities) =
            vault.joinWithToken(address(token), tokenAmount, receiver, _dynamicVaultTokensHash);
        vm.stopPrank();
    ...
    function joinWithShares(address user, address receiver, uint256 shares) internal {
        ...
        (uint256[] memory actualAssets, uint256[] memory actualLiabilities) =
            vault.joinWithShares(shares, receiver, _dynamicVaultTokensHash);
        vm.stopPrank();
        ...
        internal
    {
        ...
        (uint256 actualShares, uint256[] memory actualAssets, uint256[] memory actualLiabilities) =
            vault.exitWithToken(address(token), tokenAmount, receiver, sharesOwner, _dynamicVaultTokensHash);
        vm.stopPrank();
    ...
    function exitWithShares(address caller, address sharesOwner, address receiver, uint256 shares) internal {
        ...
        (uint256[] memory actualAssets, uint256[] memory actualLiabilities) =
            vault.exitWithShares(shares, receiver, sharesOwner, _dynamicVaultTokensHash);
        vm.stopPrank();
    ...
    function checkConvertFromShares(
        ...
        uint256 expectedLiabilities1,
        uint256 expectedLiabilities2
    ) internal view {
        ...
        for (uint256 i; i < expectedAssets.length; i++) {
            assertEq(
                ...
                expectedAssets[i],
                string.concat(
                    "convertFromShares::", DummyMintableTokenPermissionless(address(vault.assetTokens()[i])).name()
                )
            );
        }
    ...
    function checkBalanceSheet(
        ...
        for (uint256 i; i < assets.length; i++) {
            assertEq(
                ...
                expectedAssets[i],
                string.concat(
                    "balanceSheet::", DummyMintableTokenPermissionless(address(vault.assetTokens()[i])).name()
                )
            );
        }
    ...
    function checkConvertFromToken(
        ...
        for (uint256 i; i < assets.length; i++) {
            assertEq(
                ...
                expectedAssets[i],
                string.concat(
                    "convertFromToken::", DummyMintableTokenPermissionless(address(vault.assetTokens()[i])).name()
                )
            );
        }
    ...
    function checkPreviewJoinWithShares(
        ...
        for (uint256 i; i < assets.length; i++) {
            assertEq(
                ...
                expectedAssets[i],
                string.concat(
                    "previewJoinWithShares::", DummyMintableTokenPermissionless(address(vault.assetTokens()[i])).name()
                )
            );
        }
    ...
    function checkPreviewJoinWithToken(
        ...
        for (uint256 i; i < assets.length; i++) {
            assertEq(
                ...
                expectedAssets[i],
                string.concat(
                    "previewJoinWithToken::", DummyMintableTokenPermissionless(address(vault.assetTokens()[i])).name()
                )
            );
        }
    ...
    function checkPreviewExitWithShares(
        ...
        for (uint256 i; i < assets.length; i++) {
            assertEq(
                ...
                expectedAssets[i],
                string.concat(
                    "previewExitWithShares::", DummyMintableTokenPermissionless(address(vault.assetTokens()[i])).name()
                )
            );
        }
    ...
    function checkPreviewExitWithToken(
        ...
        for (uint256 i; i < assets.length; i++) {
            assertEq(
                ...
                expectedAssets[i],
                string.concat(
                    "previewExitWithToken::", DummyMintableTokenPermissionless(address(vault.assetTokens()[i])).name()
                )
            );
        }
    ...
    //NOTE: always take care not to increase the expired assets, as they're not removed from the arrays in the
    // borrowLend
    ...
    function doRollover(
        ...
        uint256[] memory assetsBalances,
        uint256 rolloverAmount0,
        uint256 rolloverAmount1
    ) public returns (address asset1New, address asset2New) {
        ...
        for (uint256 i; i < assetTokens.length; i++) {
            if (address(token) == address(assetTokens[i])) {
                assertEq(
                    ...
                    tokenAmount,
                    string.concat(
                        DummyMintableTokenPermissionless(address(token)).name(),
                        " input tokenAmount not matching derived output amount"
                    )
                );
            }
        }
...
contract DynamicOrigamiTokenizedBalanceSheetVaultTestRolloverJoinWithToken is
    DynamicTokensOrigamiTokenizedBalanceSheetVaultTestBase
{
    ...
    function test_previewJoin_halfRebalance_sharePrice_decreases() public {
        ...
        checkPreviewJoinWithToken(
            asset1, 1e18, SEED_SHARES * 2, expectedAssets, SEED_LIABILITY1 * 2, SEED_LIABILITY2 * 2
        );
        checkPreviewJoinWithToken(
            asset2, 50e6, SEED_SHARES * 2, expectedAssets, SEED_LIABILITY1 * 2, SEED_LIABILITY2 * 2
        );
    }
    ...
    function test_joinWithToken_badTokenAddress_nonSupported_asset() public {
        DummyMintableTokenPermissionless nonSupportedAsset =
            new DummyMintableTokenPermissionless("WEIRD_18dp", "WEIRD_18dp", 18);

        vm.expectRevert(
            abi.encodeWithSelector(
                IOrigamiTokenizedBalanceSheetVault.ExceededMaxJoinWithToken.selector,
                bob,
                address(nonSupportedAsset),
                10e18,
                0
            )
        );
        vault.joinWithToken(address(nonSupportedAsset), 10e18, bob, _dynamicVaultTokensHash);
    }
    ...
    function test_joinWithToken_basic_asset1New_randomRebalance() public {
        ...
        assetsBalances[1] = SEED_ASSET2 - 24_234_563;
    ...
    function test_joinWithToken_multiple_fullRebalance() public {
        ...
        joinWithToken(alice, alice, IERC20(asset1New), 61.5e18); // half of asset1 from the
        // `test_joinWithToken_basic_asset1New_fullRebalance` test
        ...
        assertEq(expectedSharesMinted, 24_600e18);
        ...
        assertEq(debt2.balanceOf(alice), 3075e6 * 2);
    }
    ...
    function test_joinWithToken_multiple_rebalance_joinWithToken() public {
        ...
        expectedAssets[1] = 6150e6 + SEED_ASSET2;
        expectedAssets[2] = 0;
        expectedAssets[3] = 0;

        checkBalanceSheet(expectedAssets, 246e18 + SEED_LIABILITY1, 3075e6 + SEED_LIABILITY2);
        ...
        expectedAssets[3] = SEED_ASSET2_NEW + 4920e6 + 39.67742e6;

        checkBalanceSheet(
            expectedAssets, SEED_LIABILITY1 + 246e18 + 30_504e18 + 246e18, SEED_LIABILITY2 + 3075e6 + 381_300e6 + 3075e6
        );

        //note: due to rebalance share are twice less expensive
        expectedSharesMinted = (6150e18 + 6150e18) + 1_525_200e18 + 12_300e18;
        assertEq(expectedSharesMinted, 1_549_800e18);

        assertEq(vault.totalSupply(), expectedSharesMinted + SEED_SHARES);
        assertEq(vault.balanceOf(alice), expectedSharesMinted);
        assertEq(asset1.balanceOf(alice), 0);
        assertEq(asset2.balanceOf(alice), 0);
        assertEq(debt1.balanceOf(alice), 246e18 * 2 + 30_504e18);
        assertEq(debt2.balanceOf(alice), 3075e6 * 2 + 381_300e6);
    }
    ...
    function test_joinWithToken_rebalance_shareIncrease_empties_vault() public {
        ...
        assetsAmounts[0] = 0;
        assetsAmounts[1] = 0;
        assetsAmounts[2] = 12_352_242_353_451_232;
        assetsAmounts[3] = 51_231;

        increaseSharePrice(assetsAmounts, 0, 59_942);

        debt2.deal(bob, 25_000_000 + 29_971); //note: this happens due to the share decrease and the ratios change, add
    ...
    function test_joinWithToken_multiple_exitWithShares() public {
        ...
        expectedAssets[1] = 6000e6 + SEED_ASSET2;

        checkBalanceSheet(expectedAssets, 240e18 + SEED_LIABILITY1, 3000e6 + SEED_LIABILITY2);
        ...
        checkBalanceSheet(assetsAmounts, 240e18 + SEED_LIABILITY1, 3000e6 + SEED_LIABILITY2);
        ...
        expectedAssets2[3] = 40e6 - 991_735;

        checkBalanceSheet(expectedAssets2, 240e18 + SEED_LIABILITY1 - 6e18, 3000e6 + SEED_LIABILITY2 - 75e6);
        ...
        expectedAssets2[3] = 40e6 - 991_735 - 1_652_892;

        checkBalanceSheet(
            expectedAssets2, 240e18 + SEED_LIABILITY1 - 6e18 - 10e18, 3000e6 + SEED_LIABILITY2 - 75e6 - 125e6
        );
    }
}

contract DynamicOrigamiTokenizedBalanceSheetVaultTestRolloverJoinWithShares is
    DynamicTokensOrigamiTokenizedBalanceSheetVaultTestBase
{
    ...
    function test_joinWithShares_multiple_fullRebalance() public {
        ...
        joinWithShares(alice, alice, 12_300e18);
        joinWithShares(alice, alice, 12_300e18);
    ...
    function test_joinWithShares_multiple_rebalance_joinWithShares_multiple() public {
        ...
        joinWithShares(alice, alice, 1_525_200e18); // half of asset1 from the
        // `test_joinWithToken_basic_asset1New_fullRebalance` test
        joinWithShares(alice, alice, 12_300e18); // half of debt2 from the `test_joinWithToken_basic_debt2` test
...
contract DynamicOrigamiTokenizedBalanceSheetVaultTestRolloverExitWithToken is
    DynamicTokensOrigamiTokenizedBalanceSheetVaultTestBase
{
    using Strings for uint256;
    ...
    function test_fullRollover_exitWithToken_basic_asset1New() public {
        ...
        expectedAssets[1] = 6150e6 - 2500e6 + SEED_ASSET2;

        checkBalanceSheet(expectedAssets, 246e18 - 100e18 + SEED_LIABILITY1, 3075e6 - 1250e6 + SEED_LIABILITY2);
        ...
        checkBalanceSheet(
            expectedAssets, 246e18 - 100e18 + SEED_LIABILITY1 + 18_204e18, 3075e6 - 1250e6 + SEED_LIABILITY2 + 227_550e6
        );
        assertEq(vault.totalSupply(), 12_300e18 - 5000e18 + SEED_SHARES + 910_200e18);
        assertEq(vault.balanceOf(alice), 12_300e18 - 5000e18 + 910_200e18);
        ...
        checkBalanceSheet(
            ...
            3075e6 - 1250e6 + SEED_LIABILITY2 + 227_550e6 - 92_500e6
        );

        assertEq(vault.totalSupply(), 12_300e18 - 5000e18 + SEED_SHARES + 910_200e18 - 370_000e18);
        assertEq(vault.balanceOf(alice), 12_300e18 - 5000e18 + 910_200e18 - 370_000e18);
    }
    ...
    function test_joinWithToken_multiple_exitWithAssets() public {
        ...
        checkBalanceSheet(
            expectedAssets2, 240e18 + SEED_LIABILITY1 - 5_999_999_999_999_999_989, 3000e6 + SEED_LIABILITY2 - 75e6
        );

        // User 4 exits with asset1 and asset2
        exitWithToken(users[4], users[4], users[4], IERC20(asset2New), 1_652_892);
        ...
        checkBalanceSheet(
            expectedAssets2,
            240e18 + SEED_LIABILITY1 - 5_999_999_999_999_999_989 - 9.999996462288184313e18,
            3000e6 + SEED_LIABILITY2 - 75e6 - 124_999_956
        );
    }
    ...
    function test_joinWithToken_multiple_shareIncrease_exitWithAssets() public {
        ...
        increaseSharePrice(assetsAmounts, 0.556825e18, 475_839);

        debt1.deal(users[2], debt1.balanceOf(users[2]) + 13_805_578_512_396_694);
        debt2.deal(users[2], debt2.balanceOf(users[2]) + 11_798);
        ...
        checkBalanceSheet(
            expectedAssets2,
            240e18 + SEED_LIABILITY1 + 0.556825e18 - 6_013_805_578_512_396_683,
            3000e6 + SEED_LIABILITY2 + 475_839 - 75_011_798
        );

        debt1.deal(users[4], debt1.balanceOf(users[4]) + 23_009_289_380_634_785);
        debt2.deal(users[4], debt2.balanceOf(users[4]) + 19_619);
        ...
        checkBalanceSheet(
            expectedAssets2,
            240e18 + SEED_LIABILITY1 + 0.556825e18 - 6_013_805_578_512_396_683 - 10_023_005_751_668_819_099,
            3000e6 + SEED_LIABILITY2 + 475_839 - 75_011_798 - 125_019_619
        );
    }
    ...
    function test_joinWithToken_multiple_shareIncrease_exitWithDebt() public {
        ...
        // User 3 exits with asset1
        exitWithToken(users[2], users[2], users[2], debt1, 6_013_805_578_512_396_683);
        ...
        // User 4 exits with asset1 and asset2
        exitWithToken(users[4], users[4], users[4], debt2, 125_019_619);
        ...
        checkBalanceSheet(
            expectedAssets2,
            240e18 + SEED_LIABILITY1 + 0.556825e18 - 6_013_805_578_512_396_683 - 10_023_005_789_679_434_558,
            3000e6 + SEED_LIABILITY2 + 475_839 - 75_011_798 - 125_019_619
        );
    }
}
