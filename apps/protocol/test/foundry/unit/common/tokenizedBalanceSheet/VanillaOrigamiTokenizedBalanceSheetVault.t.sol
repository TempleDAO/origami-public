...
contract OrigamiTokenizedBalanceSheetVaultTestJoinWithToken is VanillaOrigamiTokenizedBalanceSheetVaultTestBase {
    ...
    function test_joinWithToken_multiple() public {
        ...
        // It's benefifical for users to do large joins upfront rather than stagger them - TRUE only for Vault with Fees
        // (check test with same name in OTBSV file)

        uint256 expectedSharesMinted = 6150e18 + 6150e18;
        assertEq(expectedSharesMinted, 12_300e18);
    ...
    function test_joinWithToken_beforeShareIncrease_asset() public {
        ExpValues memory expBefore = ExpValues({ A1: 0.01e18, A2: 0.5e6, L1: 0.02e18, L2: 0.25e6 });

        ExpValues memory expAfter =
            ExpValues({ A1: 0.018064516129032258e18, A2: 0.508064e6, L1: 0.02e18, L2: 0.266129e6 });
        ...
        totalPercentageIncrease[0] =
            (((totalAssetsAfter[0] - totalAssetsBefore[0]) * 1e18) / totalAssetsBefore[0]) * 100;
        totalPercentageIncrease[1] =
            (((totalAssetsAfter[1] - totalAssetsBefore[1]) * 1e18) / totalAssetsBefore[1]) * 100;
        totalPercentageIncrease[2] =
            (((totalLiabilitiesAfter[0] - totalLiabilitiesBefore[0]) * 1e18) / totalLiabilitiesBefore[0]) * 100;
        totalPercentageIncrease[3] =
            (((totalLiabilitiesAfter[1] - totalLiabilitiesBefore[1]) * 1e18) / totalLiabilitiesBefore[1]) * 100;
        ...
        assertEq(vaultPercentageIncrease[0], 80_645_161_290_322_580_000); // 80.65%
        assertEq(vaultPercentageIncrease[1], 1_612_800_000_000_000_000); // 1.61%
        assertEq(vaultPercentageIncrease[2], 0);
        assertEq(vaultPercentageIncrease[3], 6_451_600_000_000_000_000); // 6.45%
    ...
    function test_joinWithToken_afterShareIncrease() public {
        ...
        assertEq(vault.totalSupply(), 121.173267326732673267e18 + SEED_SHARES + 608_910_891_089_108_911); //more ts
        // because of no fees, compared to the test with same name from OTBSV file.
        assertEq(vault.balanceOf(alice), 121.173267326732673267e18 + 608_910_891_089_108_911); //more shares for Alice
        // because of no fees
        assertEq(asset1.balanceOf(alice), 0);
        assertEq(asset2.balanceOf(alice), 0);
...
contract OrigamiTokenizedBalanceSheetVaultTestJoinWithShares is VanillaOrigamiTokenizedBalanceSheetVaultTestBase {
    function test_joinWithShares_unlimited_shares_capacity() public {
        ...
        vault.joinWithShares(10_000_000e18, alice, tokenHash);
    }

    function test_joinWithShares_exactlyOneShare() public {
        joinWithShares(alice, alice, 1e18);
        checkBalanceSheet(
            SEED_ASSET1 + 0.01e18, SEED_ASSET2 + 0.5e6, SEED_LIABILITY1 + 0.02e18, SEED_LIABILITY2 + 0.25e6
        );
    ...
    function test_joinWithShares_partialShare() public {
        ...
        checkBalanceSheet(
            SEED_ASSET1 + 0.009e18, SEED_ASSET2 + 0.45e6, SEED_LIABILITY1 + 0.018e18, SEED_LIABILITY2 + 0.225e6
        );
        ...
        assertEq(debt2.balanceOf(alice), 0.225e6);
    }

    function test_joinWithShares_basic() public {
        joinWithShares(alice, alice, 12_238.5e18);
        checkBalanceSheet(
            122.385e18 + SEED_ASSET1, 6119.25e6 + SEED_ASSET2, 244.77e18 + SEED_LIABILITY1, 3059.625e6 + SEED_LIABILITY2
        );
    ...
    function test_joinWithShares_multiple() public {
        ...
        joinWithShares(alice, alice, 12_238.5e18);
        checkBalanceSheet(
            122.385e18 + 122.385e18 + SEED_ASSET1, //less a1 in vault compared to fee variant, check with the same test
            // name in OTBSV file
            6119.25e6 + 6119.25e6 + SEED_ASSET2, //less a2 in vault compared to fee variant, check with the same test
            // name in OTBSV file
    ...
    function test_joinWithShares_beforeShareIncrease_asset() public {
        ...
        checkBalanceSheet(
            122.385e18 + 100e18 + SEED_ASSET1,
            6119.25e6 + 100e6 + SEED_ASSET2,
            244.77e18 + SEED_LIABILITY1,
            3059.625e6 + 200e6 + SEED_LIABILITY2
        );
        assertEq(vault.totalSupply(), 12_238.5e18 + SEED_SHARES);
        assertEq(vault.balanceOf(alice), 12_238.5e18);
        assertEq(asset1.balanceOf(alice), 0);
...
contract OrigamiTokenizedBalanceSheetVaultTestExitWithToken is VanillaOrigamiTokenizedBalanceSheetVaultTestBase {
    ...
    function test_exitWithToken_onBehalfOf() public {
        ...
        {
            checkBalanceSheet(123e18 - 1e18 - 0.1e18 + SEED_ASSET1, 6145e6, 245.8e18, 3072.5e6);
    ...
    function test_exitWithToken_basic_debt2() public {
        ...
        uint256 expectedSharesBurned = 5000e18;
    ...
    function test_exitWithToken_multiple() public {
        ...
        uint256 expectedSharesBurned = 2500e18 + 2500e18;
        assertEq(expectedSharesBurned, 5000e18);
        assertEq(vault.totalSupply(), 12_300e18 - expectedSharesBurned + SEED_SHARES);
        assertEq(vault.balanceOf(alice), 12_300e18 - expectedSharesBurned);
        assertEq(asset1.balanceOf(alice), 50e18);
        assertEq(asset2.balanceOf(alice), 2500e6);
    ...
    function test_exitWithToken_afterShareIncrease() public {
        ...
        assertEq(vault.totalSupply(), 12_300e18 - 4696.969696969696969696e18 + SEED_SHARES);
        assertEq(vault.balanceOf(alice), 12_300e18 - 4696.969696969696969696e18);
    ...
    function test_exitWithShares_fromExitWithTokenQuote() public {
        ...
        checkBalanceSheet(
            ...
            246e18 - liabilities[0] + SEED_LIABILITY1,
            3075e6 - liabilities[1] + SEED_LIABILITY2
        );
        assertEq(vault.totalSupply(), 12_300e18 - 5000e18 + SEED_SHARES);
        assertEq(vault.balanceOf(alice), 12_300e18 - 5000e18);
        ...
        assertEq(debt2.balanceOf(alice), 3075e6 - liabilities[1]);
    }
    ...
    function test_exitWithToken_fromExitWithSharesQuote() public {
        ...
        assertEq(previewShares, 5076.73634e18);
        assertEq(previewAssets[0], 50.7673634e18);
        assertEq(previewAssets[1], 2538.36817e6);
        assertEq(previewLiabilities[0], 101.5347268e18);
        assertEq(previewLiabilities[1], 1269.184085e6);

        // But doing the same with 1 less liabilities does yield the same result. note: this is not the case anymore in
        // no fee vault
        ...
        assertEq(previewShares, 5076.736336e18);
        assertEq(previewAssets[0], 50.76736336e18);
        ...
        checkBalanceSheet(
            123e18 - previewAssets[0] + SEED_ASSET1,
            6150e6 - previewAssets[1] + SEED_ASSET2,
            246e18 - previewLiabilities[0] + SEED_LIABILITY1,
            3075e6 - previewLiabilities[1] + SEED_LIABILITY2
        );
        assertEq(actualShares, sharesToExit - 3_697_169_190_258);
        assertEq(vault.totalSupply(), 12_300e18 - sharesToExit + SEED_SHARES + 3_697_169_190_258);
        assertEq(vault.balanceOf(alice), 12_300e18 - sharesToExit + 3_697_169_190_258);
        assertEq(asset1.balanceOf(alice), 50.76736336e18);
...
contract OrigamiTokenizedBalanceSheetVaultTestExitWithShares is VanillaOrigamiTokenizedBalanceSheetVaultTestBase {
    function test_exitWithShares_basic() public {
        ...
        assertEq(vault.totalSupply(), 12_300e18 - 5076.736339697169190258e18 + SEED_SHARES);
        assertEq(vault.balanceOf(alice), 12_300e18 - 5076.736339697169190258e18);
        assertEq(asset1.balanceOf(alice), 50_767_363_396_971_691_902);
        assertEq(asset2.balanceOf(alice), 2_538_368_169);
        assertEq(debt1.balanceOf(alice), 246e18 - 101.534726793943383805e18 - 1);
        assertEq(debt2.balanceOf(alice), 3075e6 - 1269.184084e6 - 1);
    }
    ...
    function test_exitWithShares_differentReceiver() public {
        ...
        assertEq(debt2.balanceOf(alice), 3075e6 - 1269.184085e6);

        assertEq(vault.balanceOf(bob), 0);
        assertEq(asset1.balanceOf(bob), 50_767_363_396_971_691_902);
        assertEq(asset2.balanceOf(bob), 2_538_368_169);
        assertEq(debt1.balanceOf(bob), 0);
        assertEq(debt2.balanceOf(bob), 0);
    }
    ...
    function test_exitWithShares_onBehalfOf() public {
        ...
        {
            checkBalanceSheet(
                123e18 - 0.1e18 - 0.5e18 + SEED_ASSET1,
                6150e6 - 5e6 - 25e6 + SEED_ASSET2,
                246e18 - 0.2e18 - 1e18 + SEED_LIABILITY1,
                3075e6 - 2.5e6 - 12.5e6 + SEED_LIABILITY2
            );
    ...
    function test_exitWithShares_multiple() public {
        ...
        assertEq(debt2.balanceOf(alice), 3075e6 - 634.592043e6 - 634.592043e6);
    }
    ...
    function test_exitWithShares_beforeShareIncrease_asset() public {
        ...
        increaseSharePrice(100e18, 100e6, 0, 200e6);
        checkConvertFromShares(1e18, 0.023655113981771456e18, 0.513655e6, 0.019999999999999999e18, 0.27731e6);

        checkBalanceSheet(
            123e18 + 100e18 + SEED_ASSET1 - 50e18 - 0.767363396971691902e18, //take fees
            6150e6 + 100e6 + SEED_ASSET2 - 2500e6 - 38.36817e6 + 1,
            246e18 + SEED_LIABILITY1 - 100e18 - 1 - 1.534726793943383805e18,
            3075e6 + 200e6 + SEED_LIABILITY2 - 1250e6 - 1 - 19.184084e6
        );
        assertEq(vault.totalSupply(), 12_300e18 - 5076.736339697169190258e18 + SEED_SHARES);
        assertEq(vault.balanceOf(alice), 12_300e18 - 5076.736339697169190258e18);
        assertEq(asset1.balanceOf(alice), 50e18 + 0.767363396971691902e18);
        assertEq(asset2.balanceOf(alice), 2500e6 + 38.36817e6 - 1);
        assertEq(debt1.balanceOf(alice), 246e18 - 100e18 - 1 - 1.534726793943383805e18);
        assertEq(debt2.balanceOf(alice), 3075e6 - 1250e6 - 1 - 19.184084e6);
    }
    ...
    function test_exitWithShares_afterShareIncrease() public {
        ...
        checkBalanceSheet(
            123e18 + 100e18 + SEED_ASSET1 - 50e18 - 40.322580645161290322e18 - 1.386204846142411179e18, //subtract the
            // fee that should have been collected
            6150e6 + 100e6 + SEED_ASSET2 - 2500e6 - 40.32258e6 - 38.987011e6,
            246e18 + SEED_LIABILITY1 - 100e18 - 1 - 1.534726793943383805e18,
            3075e6 + 200e6 + SEED_LIABILITY2 - 1250e6 - 80.645162e6 - 20.421768e6
        );
        assertEq(vault.totalSupply(), 12_300e18 - 5076.736339697169190258e18 + SEED_SHARES);
        assertEq(vault.balanceOf(alice), 12_300e18 - 5076.736339697169190258e18);
        assertEq(asset1.balanceOf(alice), 50e18 + 40.322580645161290322e18 + 1.386204846142411179e18); //add the fee
        assertEq(asset2.balanceOf(alice), 2500e6 + 40.32258e6 + 38.987011e6);
        assertEq(debt1.balanceOf(alice), 246e18 - 100e18 - 1 - 1.534726793943383805e18); //subtract the fee
        assertEq(debt2.balanceOf(alice), 3075e6 - 1250e6 - 80.645162e6 - 20.421768e6);
    }

    function test_roundTrip_checkAssetRounding() public {
        increaseSharePrice(495_032, 1359, 15, 4_294_967_293);
        ...
        uint256 tokenAmount = 4_809_252_306;
        ...
        (uint256 actualSharesJoin, uint256[] memory actualAssetsJoin, uint256[] memory actualLiabilitiesJoin) =
            vault.joinWithToken(address(token), tokenAmount, bob, tokenHash);
        vm.stopPrank();
        ...
        uint256[] memory actualAssetsExit;
        {
            ...
            (actualSharesExit, actualAssetsExit,) =
                vault.exitWithToken(address(token), tokenAmount, bob, bob, tokenHash);
            vm.stopPrank();
        }
