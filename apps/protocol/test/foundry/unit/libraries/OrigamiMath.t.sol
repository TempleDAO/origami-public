...
contract OrigamiMathTest is OrigamiTest {
    ...
    function test_scaleUp() public pure {
        assertEq(OrigamiMath.scaleUp(123.12345e6, 1e12), 123.12345e18);
        ...
        assertEq(OrigamiMath.scaleUp(123.123456e18, 1), 123.123456e18);
    ...
    function test_scaleDown() public pure {
        ...
        assertEq(OrigamiMath.scaleDown(123.123456e18, 1e12, OrigamiMath.Rounding.ROUND_DOWN), 123.123456e6);
        ...
        assertEq(OrigamiMath.scaleDown(123.123456e18, 1e12, OrigamiMath.Rounding.ROUND_UP), 123.123456e6);
        ...
        assertEq(OrigamiMath.scaleDown(123.123456e18, 1, OrigamiMath.Rounding.ROUND_DOWN), 123.123456e18);
        ...
        assertEq(OrigamiMath.scaleDown(123.123456e18, 1, OrigamiMath.Rounding.ROUND_UP), 123.123456e18);
    ...
    function test_mulDiv() public pure {
        assertEq(
            OrigamiMath.mulDiv(123.456789123456789e18, 3.123e18, 4.4567e18, OrigamiMath.Rounding.ROUND_DOWN),
            86.511443990521137174e18
        );
        assertEq(
            OrigamiMath.mulDiv(123.456789123456789e18, 3.123e18, 4.4567e18, OrigamiMath.Rounding.ROUND_UP),
    ...
    function test_muldiv_max() public {
        assertEq(
            OrigamiMath.mulDiv(type(uint256).max, type(uint256).max, type(uint256).max, OrigamiMath.Rounding.ROUND_UP),
            type(uint256).max
        );
        assertEq(
            OrigamiMath.mulDiv(
                type(uint256).max, type(uint256).max - 1, type(uint256).max, OrigamiMath.Rounding.ROUND_UP
            ),
            type(uint256).max - 1
        );
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                PRBMath_MulDiv_Overflow.selector, type(uint256).max - 1, type(uint256).max - 1, type(uint256).max - 2
            )
        );
        OrigamiMath.mulDiv(
            type(uint256).max - 1, type(uint256).max - 1, type(uint256).max - 2, OrigamiMath.Rounding.ROUND_UP
        );
        // Warning - execution of test will stop here, it's unreachable when the error is thrown via an internal
        // library.
    }
    ...
    function test_addBps_roundup() public pure {
        ...
        // 10%
        assertEq(OrigamiMath.addBps(100.123e3, 1000, OrigamiMath.Rounding.ROUND_UP), 110.136e3);

        // 33.333%
        assertEq(OrigamiMath.addBps(100.123e3, 3333, OrigamiMath.Rounding.ROUND_UP), 133.494e3);
    ...
    function test_addBps_rounddown() public pure {
        ...
        // 10%
        assertEq(OrigamiMath.addBps(100.123e3, 1000, OrigamiMath.Rounding.ROUND_DOWN), 110.135e3);

        // 33.333%
        assertEq(OrigamiMath.addBps(100.123e3, 3333, OrigamiMath.Rounding.ROUND_DOWN), 133.493e3);
    ...
    function test_subtractBps_roundup() public pure {
        ...
        // 10%
        assertEq(OrigamiMath.subtractBps(100.123e3, 1000, OrigamiMath.Rounding.ROUND_DOWN), 90.11e3);

        // 33.333%
        assertEq(OrigamiMath.subtractBps(100.123e3, 3333, OrigamiMath.Rounding.ROUND_DOWN), 66.752e3);
    ...
    function test_subtractBps_rounddown() public pure {
        ...
        // 10%
        assertEq(OrigamiMath.subtractBps(100.123e3, 1000, OrigamiMath.Rounding.ROUND_UP), 90.111e3);

        // 33.333%
        assertEq(OrigamiMath.subtractBps(100.123e3, 3333, OrigamiMath.Rounding.ROUND_UP), 66.753e3);
    ...
    function test_relativeDifferenceBps_lt_RefValue() public pure {
        assertEq(OrigamiMath.relativeDifferenceBps(100e18, 110e18, OrigamiMath.Rounding.ROUND_DOWN), 909);

        assertEq(OrigamiMath.relativeDifferenceBps(100e18, 110e18, OrigamiMath.Rounding.ROUND_UP), 910);
    }

    function test_relativeDifferenceBps_gt_RefValue() public pure {
        assertEq(OrigamiMath.relativeDifferenceBps(120e18, 110e18, OrigamiMath.Rounding.ROUND_DOWN), 909);

        assertEq(OrigamiMath.relativeDifferenceBps(120e18, 110e18, OrigamiMath.Rounding.ROUND_UP), 910);
    }

    function test_relativeDifferenceBps_eq_RefValue() public pure {
        assertEq(OrigamiMath.relativeDifferenceBps(110e18, 110e18, OrigamiMath.Rounding.ROUND_DOWN), 0);
    }
}
