...
library OlympusMonoCoolerDeployerLib {
    uint96 internal constant DEFAULT_OLTV = 2961.64e18; // [USDS/gOHM] == ~11 [USDS/OHM]
    ...
    uint256 internal constant DEFAULT_MIN_DEBT_REQUIRED = 1000e18;
    uint256 internal constant INITIAL_TRSRY_MINT = 33_000_000e18;
    uint96 public constant SUSDS_INTEREST_RATE = 0.1e18;
    ...
    function fillContractsFromTestnet(Contracts memory contracts) internal pure {
    ...
    function deploy(Contracts memory contracts, bytes32 salt, address admin, address others) internal {
        fillContractsFromTestnet(contracts);
    ...
    function updateMonoCooler(Contracts memory contracts, bytes32 salt) internal {
        // Remove from old
        contracts.kernel.executeAction(Actions.DeactivatePolicy, address(contracts.monoCooler));
    ...
    function deployExternalMocks(Contracts memory contracts, bytes32 salt) private {
    ...
    function deployOlympusCore(Contracts memory contracts, bytes32 salt) private {
        ...
        contracts.rolesAdmin = new RolesAdmin( /*{salt: salt}*/
            contracts.kernel
        );
    }

    function installModulesAndPoliciesOlympusCore(Contracts memory contracts) private {
    ...
    function setAccessOlympusCore(Contracts memory contracts, address admin) private {
    ...
    function deployMonoCooler(Contracts memory contracts, bytes32 salt) internal {
        contracts.escrowFactory = new DelegateEscrowFactory{ salt: salt }(address(contracts.gOHM));
        contracts.DLGTE =
            new OlympusGovDelegation{ salt: salt }(contracts.kernel, address(contracts.gOHM), contracts.escrowFactory);
        ...
        contracts.treasuryBorrower =
            new CoolerTreasuryBorrower{ salt: salt }(address(contracts.kernel), address(contracts.sUSDS));

        contracts.monoCooler.setTreasuryBorrower(address(contracts.treasuryBorrower));
    }

    function installModulesAndPoliciesMonoCooler(Contracts memory contracts) internal {
    ...
    function setAccessMonoCooler(Contracts memory contracts, address admin) internal {
        // Configure cooler access control
        contracts.rolesAdmin.grantRole("treasuryborrower_cooler", address(contracts.monoCooler));
        contracts.rolesAdmin.grantRole("admin", admin);
        contracts.rolesAdmin.grantRole("admin", address(this)); // This deployer added as admin such that it can enable
        // policies below
    }

    function enablePolicies(Contracts memory contracts) internal {
        contracts.treasuryBorrower.enable(bytes(""));
    }

    function seedTreasury(Contracts memory contracts, address others) private {
        // Setup Treasury
