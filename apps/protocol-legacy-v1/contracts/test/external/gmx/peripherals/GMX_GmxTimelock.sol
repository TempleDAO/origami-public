...
import "./interfaces/GMX_IGmxTimelock.sol";
...
import "../core/interfaces/GMX_IVaultPriceFeed.sol";
import "../core/interfaces/GMX_IRouter.sol";
...
contract GMX_GmxTimelock is GMX_IGmxTimelock {
    ...
    function setPriceSampleSpace(address _priceFeed,uint256 _priceSampleSpace) external onlyAdmin {
        ...
        GMX_IVaultPriceFeed(_priceFeed).setPriceSampleSpace(_priceSampleSpace);
    }
    ...
    function addPlugin(address _router, address _plugin) external onlyAdmin {
        ...
        _validateAction(action);
        _clearAction(action);
        GMX_IRouter(_router).addPlugin(_plugin);
    }
    ...
    function priceFeedSetTokenConfig(
        ...
        GMX_IVaultPriceFeed(_vaultPriceFeed).setTokenConfig(
            _token,
            _priceFeed,
            _priceDecimals,
            _isStrictStable
    ...
    function _mint(address _token, address _receiver, uint256 _amount) private {
        ...
        require(GMX_IERC20(_token).totalSupply() <= maxTokenSupply, "GmxTimelock: maxTokenSupply exceeded");
    }
